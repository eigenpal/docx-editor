...
// resolve `@docx-editor.dev/vue` through node_modules (the workspace's
// published dist/) so the deployment shows the real consumer experience.
//
// `@docx-editor.dev/core` and `/agents` live in a separate repo and are
// installed from npm, so they resolve through node_modules in both modes.
