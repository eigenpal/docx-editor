...
- [x] 2.4 **Formatting A/B falsification (reviewed KISS experiment).** Run the six direct named scenarios in `yjs-formatting-kiss.ts`: `overlap-undo`, `observed-disable`, `mark-independence`, `endpoint-affinity`, `split-tail`, and `reopen-history`. Reset deterministic per-role Yjs client IDs identically for each candidate; exchange every concurrent case in both delivery orders; assert semantic/provenance/read-only-normalization behavior plus the direct 16-record and 20,000-byte resource bounds; and measure actual source-update plus terminal-snapshot Yjs bytes with genesis excluded. A candidate is eligible only when all six scenarios pass; lower actual Yjs bytes wins; an exact byte tie selects Candidate B. The abandoned giant formatting-bakeoff corpus, seeded/frozen-corpus procedure, every-v2-limit claim, and projection-work tie-break are non-authoritative historical work. No backend migration in this task.
...
- [ ] 2.8 **Combined Y.UndoManager actor history and acceptance.** Integrate one public `Y.UndoManager` per actor+session scoped to `bodySequence` plus `markContributions`, stable origin token, explicit `stopCapturing`, same-session manager-stack redo semantics, bounded reconstruction journal, durable reopen, and local behavioral parity; pass G-v2-1..G-v2-10 and gate 8. G-v2-6 covers winner formatting endpoints only; selection and annotation endpoints remain owned by tasks 3.2 and 4.3. Depends on 2.7. Allocator, audit, awareness, capsules, and repair metadata remain untracked.
