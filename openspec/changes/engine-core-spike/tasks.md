...
- [x] 2.5 **Reviewed v2 oracle (no implementation).** Freeze `yjs-schema.v2.json`, `binding-oracle.v2.json`, `history-oracle.v2.json`, and `comparator-contracts.v2.json` around only the task 2.4 winner and its `FormattingEvidence`, plus G-v2-1..G-v2-10, plain JSON length-1 embeds, canonical-base64url relative envelopes and rejection/detach rules, winner identity/provenance semantics, boundary collisions, monotonic repair keying, manager-stack redo rules, every closed limit, and independent oracle hashes. Remove all loser-only roots, fields, and comparators. Depends on 2.4.
