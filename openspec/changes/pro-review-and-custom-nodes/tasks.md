...
- [ ] 4.5a Per-line boundary fragments: a line-wrapped inline control gets ONE union rect from layout, so chip styling can cover neighboring words (owner screenshot 2026-08-05); fragments must split per line
...
- [~] 4.6 Atomic offsets, first half: typing at a control's outer edge no longer joins the run inside it (`leavesContainer` treats `contentControl` like `hyperlink`; pinned by test). REMAINING: caret skip-as-unit, whole-SDT backspace/delete, copy/paste carries OOXML
- [ ] 4.6b Boundary-rect vertical offset under non-single line spacing (chip tints the leading above the text, click lands under it — owner screenshots 2026-08-05); `CustomNodeChrome` ships a downward hit-test fallback, the layout fix belongs with 4.5a
