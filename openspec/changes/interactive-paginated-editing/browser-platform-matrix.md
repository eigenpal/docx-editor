...
| --- | --- |
| Direct pointer click/drag on the **paginated interaction surface** | **React: automated and passing** (`test:e2e:react-one-surface-interaction`). **Vue: not automated.** Still does not satisfy public `interactive-paginated` — that remains task **8.10** |
| Typed editing on the **paginated interaction surface** (caret on painted pages) | **React: automated and passing** — click places the caret, typing commits to the canonical model and repaints. **Vue: not automated.** Still does not satisfy public `interactive-paginated` |
