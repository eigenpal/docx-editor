...
- [ ] 10.2 Consolidate the core contract and semantic store lanes under `packages/core/src/contracts` and `packages/core/src/store`, establishing `@docx-editor.dev/core` as the implementation package. — PARTIAL: the store lane has moved to `packages/core/src/store` with its tests, and `packages/engine-core` is now the compatibility alias 10.5 permits while a lane is in flight. The CONTRACT half is not done: the contract modules still sit at `packages/core/src/*.ts` rather than `packages/core/src/contracts/`, which is where the lane DAG says they belong. Two boundaries also weakened and are not yet restored: the store lane now typechecks under core's tsconfig, which includes the DOM lib (`engine-core` deliberately did not), and its tests are excluded from typecheck exactly as they were before the move — they have never been typechecked and do not currently pass a typecheck. Both need task 10.1's per-lane TypeScript project boundaries.
- [ ] 10.3 Consolidate binding, layout, output, and browser editor composition under guarded `packages/core/src/{binding,layout,output,editor}` lanes without widening their allowed dependencies. — PARTIAL: `layout`, `output` and `binding` have moved, each with its tests, reached at its own subpath, with `packages/engine-{layout,output,binding}` left as the aliases 10.5 permits. The `editor` lane is BLOCKED and was reverted: `packages/core/package.json` already exports `./editor` as the public CONTRACT (`src/editor.ts`), which 78 files import as `@docx-editor.dev/core-contract/editor`. Giving the editor lane its DAG-declared `./editor` subpath would silently repoint all 78 at the lane. It needs the contract half of 10.2 first — relocate `src/editor.ts` into `src/contracts/` and migrate those importers (10.5) — after which `./editor` is free. Also note two dependency widenings that are real: `pdf-lib` (output) and the ProseMirror packages (binding) now sit in the single core manifest, so per-lane dependency claims can no longer be made against a manifest and are checked against the import graph instead.
...
- [ ] 10.5 Migrate React, Vue, Nuxt, agents, scripts, tests, and documentation to intentional `@docx-editor.dev/core` subpath imports using temporary compatibility aliases only while a lane is in flight.
- [ ] 10.6 Remove each `packages/engine-*` workspace package only after its typecheck, tests, runtime graph, and API surface pass through `packages/core`.
