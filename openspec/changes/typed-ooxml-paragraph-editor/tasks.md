...
- [ ] 7.7 Make line metrics EXACT rather than measured-and-approximated. Word derives single spacing from the font's `hhea` ascent + descent + line gap; a browser measurement cannot report the line gap, so any host-side measurer is a fraction out and every run either overflows its line or leaves a gap under it. — DONE for the metric source: `createShapedMeasurer` reads the real tables through the existing HarfBuzz shaper, so advances are summed glyph advances and line height is Word's own formula; the private harness shapes and paints the SAME font bytes, registering them with `FontFace`, so measured and painted advances cannot drift. REMAINING, and the ORDER matters: (1) fonts EMBEDDED in the document — `word/fontTable.xml` names them and `w:embedRegular`/`w:embedBold` point at the font parts, which Word obfuscates with a GUID key that has to be undone before the bytes are a usable face. A document that carries its own fonts needs no substitute and no network. (2) Only where a family is neither embedded nor installed, resolve it dynamically. Hardcoding one demo face, as the harness does today, is a bench device and must not become the product behaviour, and a fixture asserting line breaks and page count against RECORDED WORD OUTPUT rather than against the engine's own measurer — the latter needs Word-produced reference data this repo does not yet carry.
