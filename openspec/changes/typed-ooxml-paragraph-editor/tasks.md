...
- [x] 7.3 Resolve paragraph style, alignment, spacing, line spacing/rule, left/right/first-line/hanging indents, tabs, numbering identity/level, keep-next, keep-lines, widow control, page-break-before, and shading into paragraph fragments and pagination. — EXTENDED (core engine, not yet public): `w:spacing` before/after with collapse, `w:pBdr/w:bottom` only, typed `w:br w:type="page"` layout + save/reopen, section-aware pagination (per-section geometry/margins, default/`nextPage`, `titlePage`, per-section read-only HF inheritance), and bounded allowlisted complex `PAGE`/`NUMPAGES` projection for read-only page-furniture numbering. Deferred: body fields (`w:fldSimple` stays layout-inert — cached results are not tree-ops-addressable), other `w:pBdr` edges, `continuous`/`evenPage`/`oddPage` semantics, multi-column flow, section insertion UI, product commands, and paired acceptance/public claims (11.4).
