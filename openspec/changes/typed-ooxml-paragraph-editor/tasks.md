...
- [ ] 10.1 Define the guarded internal lane DAG, TypeScript project boundaries, conditional subpath exports, and browser/server bundle-graph checks under `packages/core`. — PARTIAL: the lane DAG, the subpath exports and the browser bundle-graph walk all exist and are checked. The TypeScript PROJECT BOUNDARIES do not, and their absence is now load-bearing: one tsconfig covers every lane and must include `DOM`/`DOM.Iterable` for the browser lanes, so a DOM call in a NEUTRAL lane (store, layout, sync, clients) would compile. Previously each neutral package's own tsconfig omitted the DOM lib and made that a compile error. What remains is the static DOM-usage scan in layout-authority, which is weaker. Also unrestored: every moved lane's tests stay excluded from typecheck, as they were in their separate package projects, and do not currently pass one.
