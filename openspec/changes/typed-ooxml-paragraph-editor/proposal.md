...
- Prove the slice first through a private React acceptance harness, then require React/Vue production parity before exposing it as supported behavior.
...
The change governs `packages/engine-core`, `packages/engine-binding`, `packages/engine-layout`, `packages/engine-output`, `packages/engine-editor`, and the React/Vue adapters. It resets OpenSpec authority and documentation references without restoring archived proposals or changing public support claims until the parity gates pass.
