...
Tables with horizontal or vertical merges disable column insertion, column deletion, and column resizing with a specific refusal reason. A row inserted at a boundary inside an active vertical-merge chain repeats `w:vMerge` for the covered cells, so the covering cell grows by one row; a row whose grid mapping the walk cannot read is refused with a specific reason. Each operation validates its complete target before mutation, publishes one `flow-structural` or property impact, and creates one undo step. Edited XML parts serialize in normalized form; canonical fingerprints and save/reopen semantic digests prove structural preservation.
