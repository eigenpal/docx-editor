...
- **AND** with `@layoutInCell="0"` they resolve against the page or margin as declared, and the object carves no side wrap hole in the cell (top-and-bottom still moves the cell text below it), only when the document's compatibility mode is 14 or lower or absent
