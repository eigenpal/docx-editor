...
| Piece                                                         | Where                                   | Notes                                                       |
| ------------------------------------------------------------- | --------------------------------------- | ----------------------------------------------------------- |
| `<DocxEditor agentPanel>`                                     | `@docx-editor.dev/react`                | Right-hand panel, sparkle toolbar toggle, drag-resize       |
| `<AgentChatLog>`, `<AgentComposer>`, `<AgentTimeline>`        | `@docx-editor.dev/agents/react`         | Opinionated chat primitives — optional                      |
| `useDocxAgentTools()`                                         | `@docx-editor.dev/agents/react`         | Tool executor + selection / page context                    |
| `getToolSchemas()`, `getToolDisplayName()`, `executeToolCall` | `@docx-editor.dev/agents/server`        | OpenAI function-calling format — runtime-agnostic           |
| `getAiSdkTools()`                                             | `@docx-editor.dev/agents/ai-sdk/server` | **AI SDK only.** Returns `streamText({ tools })` shape      |
| `toAgentMessages()`                                           | `@docx-editor.dev/agents/ai-sdk/react`  | **AI SDK only.** `useChat` `UIMessage[]` → `AgentMessage[]` |
| `DocxReviewer`                                                | `@docx-editor.dev/agents` (Node)        | Headless: same toolkit, no editor                           |
...
```bash
npm i @docx-editor.dev/react @docx-editor.dev/agents \
...
```ts
import { getToolSchemas, executeToolCall } from '@docx-editor.dev/agents/server';
...
import { getToolSchemas } from '@docx-editor.dev/agents/server';
