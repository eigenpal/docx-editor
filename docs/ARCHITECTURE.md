...
│  └────────────────┘  └──────────────┘  │  └────────┘ └────────┘ │  │
│         ↓                   ↓          └────────────────────────┘  │
│    EditorState      layOutPages()           DOM Pages           │
...
```
src/layout-engine/
├── index.ts              # Main layOutPages() export
├── types.ts              # Layout, Page, Fragment, Measure types
├── pageComposer.ts          # Page/column creation and tracking
├── sectionRules.ts     # Section property handling
...
type FlowBlock = ParagraphBlock | TableBlock | ImageBlock | SectionMarkerBlock | PageBreakBlock;

type Measure = ParagraphMetrics | TableMeasure | ImageMeasure | SectionBreakMeasure;
...
type ParaFragment = {
  ...
  fromLine: number;
  toLine: number;
  docFrom?: number; // ProseMirror position mapping
  docTo?: number;
};
```

**Core Algorithm (`layOutPages`):**

```typescript
function layOutPages(blocks: FlowBlock[], measures: Measure[], options: LayoutOptions): Layout {
  // 1. Initialize pageComposer with page size, margins
...
├── pm-adapter/
│   ├── buildBoxTree.ts      # PM Doc → FlowBlock[] conversion
│   ├── fromFlowBlocks.ts    # FlowBlock[] → PM Doc (if needed)
│   └── types.ts             # Adapter types
├── measuring/
│   ├── paragraphLayout.ts  # DOM measurement for text
...
├── position-mapping/
│   ├── pointerToDocPos.ts   # (x,y) → PM position
│   ├── positionToRect.ts    # PM position → screen rect
│   └── selectionGeometry.ts    # Selection → rectangles
└── worker/
...
function convertParagraph(
  ...
  return {
    ...
    attrs: {
      docFrom: baseOffset,
      docTo: baseOffset + node.nodeSize,
...
function paintPage(
  ...
  for (const fragment of page.fragments) {
    const fragmentEl = paintFragment(fragment, blocks, measures, context);
...
│   └── SelectionGeometry.ts    # PM selection → screen rects
├── headers-footers/
...
async function paragraphLayout(
...
│                       PM → FLOW BLOCKS                               │
│                                                                      │
│  buildBoxTree(pmDoc) → FlowBlock[]                                   │
│  • Paragraphs → ParagraphBlock with runs                             │
│  • Tables → TableBlock with cells                                    │
│  • Each block has docFrom/docTo for position mapping                 │
└─────────────────────────────────────────────────────────────────────┘
                                  │
                                  ▼
┌─────────────────────────────────────────────────────────────────────┐
│                      TEXT MEASUREMENT                                │
│                                                                      │
│  paragraphLayout(block, { maxWidth }) → ParagraphMetrics            │
...
│                      LAYOUT ENGINE                                   │
│                                                                      │
│  layOutPages(blocks, measures, options) → Layout                  │
...
function handleClick(event: MouseEvent) {
  ...
  const pageHit = locatePageHit(layout, { x: clientX, y: clientY });
  if (!pageHit) return;

  // 3. Find which fragment was clicked
  const fragmentHit = resolveFragmentHit(layout, pageHit, blocks, measures, {
...
function rectsForSelection(
  ...
  for (let pageIndex = 0; pageIndex < layout.pages.length; pageIndex++) {
    ...
    for (const fragment of page.fragments) {
      ...
      const docFrom = fragment.docFrom ?? 0;
      const docTo = fragment.docTo ?? docFrom;
      ...
      const fragmentRects = getSelectionGeometryInFragment(
        fragment,
        block,
        measure,
        Math.max(from, docFrom),
        Math.min(to, docTo),
        pageIndex
      );
...
async function layoutWithHeadersFooters(
  ...
  return layOutPages(blocks, measures, layoutOptions);
}
```
...
- [ ] Implement basic `layOutPages()` for paragraphs only
- [ ] Create `PageComposer` class for page management
...
- [ ] Create `buildBoxTree()` conversion from ProseMirror
- [ ] Add docFrom/docTo tracking to all blocks
...
5. **PM Position Tracking**: Every fragment knows its docFrom/docTo positions, enabling bidirectional mapping between clicks and PM state.
