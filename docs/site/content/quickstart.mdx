...
For server-side rendering (SSR) frameworks such as Next.js, Nuxt, and Remix, render the editor only on the client. Use the client-only setup in [Installation](/docs/2.x/installation). This example works in client-rendered apps such as Vite.
...
- `mode` is `'edit'` (default), `'view'`, or `'suggesting'`. Changes apply without a remount. Suggesting mode requires a review module and a non-empty `author`.
...
To react to the built-in Save action (Command+S or **File > Save**) instead of adding your own button, handle the save event. It replaces the packaged behavior, so read the bytes from the ref:
