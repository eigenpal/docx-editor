...
`useHistoryGroup({ kind })` binds a control's native gesture events. It returns
`ref` and `options()`. The core controller owns boundaries; the hook cleans up
when the element or editor changes and when the component unmounts.

Use `range` for sliders, `repeat` for held buttons, `keyboard` for keyboard
controls, and `native-color` for native color commitments. This color control
applies React `onChange` values without ending each intermediate frame:
    ...
    <label title={color.disabledReason ?? undefined}>
      ...
      <input
        ...
        onChange={(event) => {
          ...
          setStatus(result.ok ? (result.history?.kind ?? 'none') : result.reason);
        }}
      />
...
`useEditorValueCommand` also accepts `font.family`, `font.size`,
`text.highlight`, `styles.style`, and `list.lineSpacing`. Font size uses
half-points. `execute(value, options?)` returns `ExecResult`; `value` stays
synchronized with selection changes and undo. Null means mixed or unavailable.
Image value slots accept options but refuse a history group.

`useEditorCommand().execute(options)` retains its boolean result. Its nullary
form remains safe as `onClick={command.execute}`; event arguments are ignored.
