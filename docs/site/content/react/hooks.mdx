...
| `useReviewAuthors()`              | Every review author the current view draws — tracked-change authors first, comment-only authors after — with the color and style each resolves to. See [Tracked changes](/docs/2.x/pro/tracked-changes). |
| `useEditorCaret()`                | Caret as `{ paragraphId, offset }` for write APIs that take an `at` position.              |
| `useZoom()`                       | Read and set zoom from custom chrome.                                                      |
...
| `useToolbarLabelFor(slotId)`      | Label resolver for one slot id.                                                            |
...
Review hooks (`useReview`, `useReviewOf`, `useReviewItem`, `useReviewAuthor`) ship in [`@docx-editor.dev/pro/react`](/docs/2.x/pro). The Vue twin lives in [`@docx-editor.dev/pro/vue`](/docs/2.x/pro).
