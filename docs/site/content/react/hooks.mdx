...
Use `useCollaborationStatus` with an externally owned collaboration session.
The hook returns `inactive` when you pass `null`:

```tsx
import { useCollaborationStatus } from '@docx-editor.dev/react';
