...
but one gesture takes one Undo press. Redo restores the final value.

Render the control inside `DocxEditor.Root`, just like other editor hooks.
...
selection, including Undo and Redo, so your app does not need a second copy of
the document's formatting state.
...
Drag the slider twice, then press Undo. The first Undo returns to the end of
the first drag. The second returns to the original formatting. A held arrow
key also forms one gesture, ending when you release it.

The binding handles outside pointer release, cancellation, lost capture, and
blur. It retains the gesture across rerenders and disposes it on unmount or
...
| -------------- | ------------------------------------------------ | --------------------------------------------------------- |
| `range`        | A native range input or custom slider            | Pointer release/cancel, lost capture, keyup, or blur      |
| `repeat`       | A button that applies repeated values while held | Pointer release/cancel, lost capture, keyup, or blur      |
| `keyboard`     | A control driven by repeated keydown events      | Keyup, Escape, or blur                                    |
| `native-color` | An HTML color input                              | Native value commitment, next activation, Escape, or blur |
...
accept six hexadecimal digits without `#`. React `onChange` applies the live
values; the binding listens for the native commitment event separately.
...
value; it does not guarantee that a popup closed. If a browser sends no terminal
commitment event, the next activation, Escape, blur, or unmount ends the gesture.
Use a custom slider when you need exact drag boundaries across browsers.
...
image commands refuse a history group.
...
Use `useDocxEditor().exec(command, options)` when you need the full result for a
raw command; check that the editor exists before calling it.

For optional development feedback, subscribe with
`useEditorEvent('historyDiagnostic', handler)`. The editor reports interrupted
groups and possible accidental per-frame groups. Diagnostics do not change
history behavior or log automatically.
