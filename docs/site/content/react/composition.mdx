...
Choose toolbar controls, placement, labels, spacing, and a visual style. Add optional document controls. The source code includes the selected components and CSS.

These example layouts use the composition API; they are not packaged toolbar presets. The demo shows a subset of the available components and options. Use your own React components and TSX for toolbars, menus, and panels. Connect them with [React hooks](/docs/2.x/react/hooks), or combine them with packaged parts.
