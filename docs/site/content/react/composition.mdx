...
Creation parameters are sampled when the instance is created. Changing the identity of `document`, `fonts`, or `imageDecodePort` remounts the editor, as does switching the locale catalog. Later `zoom` and `zoomMode` changes apply live through the facade, so edits, the caret, and the undo history survive them.

| Prop                    | Type                                                             | Description                                                                                                                                                             |
| ----------------------- | ---------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `document`              | `DocumentSource`                                                 | DOCX bytes or an existing `DocumentHandle`. Identity change remounts.                                                                                                   |
| `fonts`                 | `FontConfiguration \| FontConfigurationFragment \| FontResolver` | Font bytes for Word-accurate wrap and pagination, or a resolver called per load.                                                                                        |
| `author`                | `string`                                                         | Ambient author for comments and tracked edits. Required for suggesting.                                                                                                 |
| `locale`                | `string`                                                         | Locale passed to the underlying editor instance.                                                                                                                        |
| `mode`                  | `'edit' \| 'view' \| 'suggesting'`                               | The opening mode. Unlike `<DocxEditor>`, `Root` has no default: when you omit it, the document decides, so a file carrying `w:trackRevisions` opens in suggesting mode. |
| `modules`               | `EditorModule[]`                                                 | Capability modules, registered at construction. Keep the array identity stable.                                                                                         |
| `zoom` / `zoomMode`     | `number` / `ZoomMode \| 'auto'`                                  | The display scale and where it comes from. `'auto'` fits the page width.                                                                                                |
| `onReady`               | `(editor: Editor) => void`                                       | Fired once per instance, after any `Content` in the same commit has attached.                                                                                           |
...
| `translate`             | `(key, params?) => string`                                       | Resolver for painted-surface labels; defaults to the active locale catalog.                                                                                             |
| `tableInteractionLabel` | `(key) => string`                                                | Labels for table insertion furniture on the painted surface.                                                                                                            |
| `imageDecodePort`       | `ImageDecodePort`                                                | Overrides raster decode for image insertion in tests or custom hosts.                                                                                                   |

For more information, see the [full API reference](/docs/2.x/api/react).
...
- `DocxEditor.AuthorStyle` and `DocxEditor.ColorByChangeType`: render-nothing declarations that give one author a specific look, or color tracked changes by change type instead of by author. See [Tracked changes](/docs/2.x/pro/tracked-changes).
