...
For cards that share nothing with the packaged layout, `preset={false}` mounts the rail and its context without the packaged arrangement, so you keep the subscription and the anchoring and render the cards yourself. `useStackedReviewPositions(items, heights, { gap, scale })` is the packaged stacking math if you want it — pass the editor's render scale, or card heights in pixels get compared against anchors in points. Below that, `useReview()` gives you the queue with no chrome at all.
