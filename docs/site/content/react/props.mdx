...
Configure automatic dialogs on the editor or `Root` with `pageSetup`, `paragraph`,
and `textFormField` render callbacks. Forward their supplied props to the dialog.
Omitted callbacks use defaults. For examples and callback props, see
[Customize dialogs](/docs/2.x/guides/customize-dialogs).
