...
<DocxEditor documentBuffer={buf} fonts={fonts} onFontError={(e) => report(e.code)} />;
```

`fonts` is sampled at mount and its identity change remounts the editor, so pass a
stable reference — resolve it once and hold it in state rather than building it inline.
...
For host code that isn't using the React or Vue adapter, `@docx-editor.dev/core/utils` exports `loadFontFromUrl`, `loadFontDefinitions`, `onFontError`, and the `FontDefinition` type so you can drive the same registration directly.
...
| Prop | Type | Description |
|---|---|---|
| `i18n` | `LocaleStrings \| PartialLocaleStrings` | Locale data from `@docx-editor.dev/i18n`. See [i18n page](/docs/1.x/i18n). |
