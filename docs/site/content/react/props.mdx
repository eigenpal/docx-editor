...
Use `definePopup(MyPopup)` to customize an automatic popup with your component.
Forward its typed props to the default component to retain editor behavior.
Render callbacks also work. Set an entry to `false` to manage its trigger and visibility yourself.
For supported popups and defaults, see [Customize popups](/docs/2.x/guides/customize-dialogs).
