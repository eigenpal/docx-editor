...
Use `loadDefaultFonts()` to load the default font set:
...
<DocxEditor document={bytes} fonts={fonts} onFontError={(error) => report(error.code)} />;
```

The editor samples `fonts` at mount.
Changing its identity remounts the editor.

You can pass a resolver function.
The editor calls it once per load with the declared font families.
Use `useFonts` to keep the resolver identity stable:
