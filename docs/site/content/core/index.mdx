...
A live control applies every intermediate value while the user drags it, so the
selection updates under the pointer. Each call to `exec` is one transaction, and
by default each transaction is one undo step. To make a whole gesture one undo
step, pass the same `historyGroup` token with every call of that gesture:
...
Every call still renders and replicates on its own. Undo restores the formatting
from before the first call, exactly as it was, so a selection with mixed colors
comes back mixed. Redo restores the last value applied.

The token is the boundary. Create a new token for each gesture, and the engine
closes the group when any of the following happens:

- A call arrives without a token, or with a different token.
- A call lands in a different story, such as a header after the body.
- Buffered typing flushes ahead of a call. The keystrokes are their own step.
- The user runs undo or redo.
- A command records a whole-package unit, such as inserting an image or a footnote.

A call that changes nothing adds no undo step and leaves the group open. Every
formatting and text command accepts the option, so a font-size stepper, a
spacing slider, or an indent control that you build groups the same way.
Review, protection, content-control, and form-field commands always record
their own step, and image commands refuse the option. A control that
dispatches by slot passes the same options through `runToolbarCommand`. The
packaged toolbar does not pass a token.

In a collaborative session, the shared undo manager owns undo. The token groups
there too: frames of one gesture join one shared undo item however long the
gesture lasts, and a change of group starts a new item. A story change inside
one gesture does not split the shared item. Inserting a hyperlink publishes its
relationship as a separate shared item. Calls without a token keep the
manager's own rule, which joins local edits inside its capture window.
