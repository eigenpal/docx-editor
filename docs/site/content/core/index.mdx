...
description: "The framework-agnostic engine bundled into the React adapter: OOXML read and write, the canonical document tree, layout, paint, and the Editor contract."
category: "Reference"
order: 70
---
...
You do not install this package. It is bundled into `@docx-editor.dev/react`, which re-exports the contract types you write signatures against:

```ts
import type { Editor, EditorCommand, EditorSnapshot, PageSetup } from '@docx-editor.dev/react';
```

This page describes what that engine does and how it is organized, so you can reason about fidelity, performance, and the security boundary.

## What the engine contains

The areas below are internal structure, not import paths.
...
| `./contracts/plugin`      | The plugin contract.                                                                                                         |
| `./contracts/interaction` | Pointer, selection, and keymap contracts.                                                                                    |
...
```ts
import type { Editor, EditorSnapshot } from '@docx-editor.dev/react';
