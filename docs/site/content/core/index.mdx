...
formatting and text command accepts the option, so font-size steppers, spacing
sliders, and indent controls group the same way. Accepting or rejecting
revisions, applying a form field, and refreshing fields always record their own
step. A packaged control groups through `runToolbarCommand`, which forwards the
same options.
...
gesture lasts, and a change of group starts a new item. Calls without a token
keep the manager's own rule, which joins local edits inside its capture window.
