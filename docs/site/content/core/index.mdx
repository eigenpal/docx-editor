...
A call that changes nothing adds no undo step and leaves the group open. Any
command that edits a story accepts the option, so font-size steppers, spacing
sliders, and indent controls group the same way.
