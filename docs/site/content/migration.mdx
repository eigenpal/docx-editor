...
0.x was a single package. 1.x splits it into five npm packages by concern (plus `@docx-editor.dev/nuxt`, a Nuxt module wrapping the Vue adapter):

| What you used in 0.x | Where it is in 1.x |
|---|---|
| The `<DocxEditor>` component, toolbar, dialogs, hooks, plugin host | `@docx-editor.dev/react` (or `@docx-editor.dev/vue`) |
| The document model and OOXML parse/serialize (`Document`, `Paragraph`, `createEmptyDocument`, ...) | `@docx-editor.dev/core` |
| Anything AI: the reviewer, agent UI components, the MCP server | `@docx-editor.dev/agents` |
| Locale strings (`pl`, `de`, `pt-BR`, ...) | `@docx-editor.dev/i18n` |

In practice a React app still imports from one package, `@docx-editor.dev/react`, which pulls in `-core` and `-i18n` for you. You only name the other packages when you import the document model, the agent toolkit, or locale data directly. The rest of this guide is the symbol-level detail behind that split.
...
3. Change every remaining `@eigenpal/docx-js-editor` import to `@docx-editor.dev/react`.
...
The monolithic `@eigenpal/docx-js-editor` split into five packages under the `@eigenpal/docx-editor-*` namespace, plus the `@docx-editor.dev/nuxt` module.

| Old (0.x) | New (1.x) |
|---|---|
| `@eigenpal/docx-js-editor` | `@docx-editor.dev/react` |
| `@eigenpal/docx-js-editor/core` | `@docx-editor.dev/core` |
| `@eigenpal/docx-js-editor/mcp` | `@docx-editor.dev/agents/mcp` |
| `@eigenpal/docx-js-editor/i18n/<code>.json` | `@docx-editor.dev/i18n/<code>` (subpath) or named export off the root |
| `@docx-editor.dev/agents` (AGPL-3.0) | `@docx-editor.dev/agents` (Apache 2.0) |
...
Five categories of symbols moved across packages (not just subpaths). A plain package-name swap to `@docx-editor.dev/react` won't resolve these. **If a symbol from this list lives in your code, point its import at the new home below; otherwise every one of them becomes a "no exported member" error.**

| Symbol(s) | Was (0.x) | Now (1.x, recommended) |
|---|---|---|
| `createEmptyDocument`, `createDocumentWithText`, `CreateEmptyDocumentOptions` | `@eigenpal/docx-js-editor` | `@docx-editor.dev/react` or `@docx-editor.dev/vue` (re-exported in 1.0.1 from `-core`) |
| Document-tree types most apps reach for: `Document`, `Comment`, `Paragraph`, `Run`, `Insertion`, `Deletion`, `TrackedChangeInfo`, `Hyperlink`, `Table` | `@eigenpal/docx-js-editor` | `@docx-editor.dev/core` (root entry surfaces them in 1.0.1) |
| Rarer document-tree types: `Image`, `Footnote`, `Endnote`, `TableRow`, `TableCell` | `@eigenpal/docx-js-editor` | `@docx-editor.dev/core` (root entry; also on `/headless`) |
| Agent UI components: `AgentPanel`, `AgentChatLog`, `AgentComposer`, `AgentSuggestionChip`, `AgentTimeline` (and their `*Props` types) | `@eigenpal/docx-js-editor` | `@docx-editor.dev/agents/react` (or `/vue`) |
| `PluginHost`, `PluginHostProps`, `PluginHostRef`, `templatePlugin` | `@eigenpal/docx-js-editor` | `@docx-editor.dev/react/plugin-api` |
...
Once the moved symbols are handled, every remaining `@eigenpal/docx-js-editor` import changes to `@docx-editor.dev/react`. Only the package name changes; the imported names stay the same.
...
| `@eigenpal/docx-js-editor/core` | `@docx-editor.dev/core` |
| `@eigenpal/docx-js-editor/mcp` | `@docx-editor.dev/agents/mcp` |
| `@eigenpal/docx-js-editor/i18n/<code>.json` | `@docx-editor.dev/i18n` (see [i18n restructure](#i18n-restructure) below) |
...
+ import { Toolbar } from '@docx-editor.dev/react/ui';
```
...
+ import { EditorToolbar } from '@docx-editor.dev/react/ui';
// or compose your own:
+ import { Toolbar, ToolbarButton, ToolbarGroup } from '@docx-editor.dev/react/ui';
```
...
import { pl, ptBR } from '@docx-editor.dev/i18n';
```
...
+ import(`@docx-editor.dev/i18n/${locale}`).then((m) => setI18n(m.default));
```
...
+ import type { LocaleStrings, PartialLocaleStrings } from '@docx-editor.dev/i18n';
+ type LocaleI18n = LocaleStrings | PartialLocaleStrings | undefined;
```

## License change

The 0.x line of `@docx-editor.dev/agents` was AGPL-3.0. In 1.0, every published package is **Apache 2.0**. If you tracked license obligations for compliance review, update the entry. Apache 2.0 is permissive: no copyleft on derivative works.
