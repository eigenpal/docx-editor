...
`getDocumentWatermark` and `setDocumentWatermark` from `@docx-editor.dev/core/headless` read and write the same model:

```ts
import { parseDocx, setDocumentWatermark, repackDocx } from "@docx-editor.dev/core/headless";
