...
A field can also carry a numeric picture switch, such as `PAGE \# 0#`, which pads page 2 to `02`. The editor evaluates the picture against the computed value, so a stale result saved into the file never reaches the page. Pictures support digit placeholders, a grouping comma, and literal text.
