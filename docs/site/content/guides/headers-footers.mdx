...
Other fields use separate evaluation rules. See [Fields and cross-references](/docs/2.x/guides/fields).
...
A right-aligned `PAGE` frame can also sit over the footer text line. The frame uses `w:wrap="none"`, `w:hAnchor="margin"`, and `w:xAlign="right"`, and the next paragraph holds plain text in the same style. The page value ends at the right margin, one line band with the text. The frame adds no footer height unless its own line is taller. The text paragraph keeps its indents and its style's spacing. If the text reaches under the page value, the footer keeps the ordinary flow behavior.

This support covers a simple, single-line `PAGE` frame and a single-line empty, `PAGE`, or text anchor. Values must fit completely inside the frame or lie completely beyond it. Partial wrapping, other positions, direct paragraph spacing, and additional content keep the ordinary flow behavior.
...
- **Different odd and even pages** (`w:evenAndOddHeaders`) alternates `default` and `even` variants, as in book-style layouts. If a section restarts its page numbering and the next sheet has the other parity, an empty sheet with no header or footer comes first.
...
Word stores watermarks in header parts. The editor renders the supported unrotated [legacy VML subset](/docs/2.x/guides/images#legacy-vml-previews) there. Rotated or curved watermark templates remain opaque.
