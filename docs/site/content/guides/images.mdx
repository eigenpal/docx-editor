...
- The picture is not rotated or flipped, and it has a rectangle geometry and a stretched fill.
...
If a condition is false, the editor treats the group as an unsupported group. Picture commands, such as resize and replace, do not apply to a picture in a group. Saving preserves the group, its alternative content, and its media without changes. An embedded picture passes the normal image validation and decode limits. A linked picture is never fetched.
