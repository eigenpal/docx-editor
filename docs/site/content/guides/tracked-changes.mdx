...
import { DocxEditor, type EditorMode } from '@docx-editor.dev/react';
...
The accept/reject operations behind the sidebar buttons are exported as ProseMirror commands from `@docx-editor.dev/core/prosemirror/commands`:
...
To enumerate revisions (for your own list UI or an approval workflow), use `extractTrackedChanges` from `@docx-editor.dev/core/prosemirror/utils/extractTrackedChanges`. It takes the editor state and returns the revision entries plus a comment-to-revision map.
...
It returns `false` when the paragraph or search text is not found. This is the primitive automated redlining builds on; see [DocxEditor API](/docs/1.x/editor-api) for driving it from code.
...
- [DocxEditor API](/docs/1.x/editor-api) for proposing tracked changes from code
