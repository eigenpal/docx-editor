...
`defaultFonts()` and `packagedFonts()` supply bytes without changing public CSS
family names. The editor registers private aliases for paint. This preserves native
glyph fallback and the host application's fonts.

Use `installDefaultFontFaces()` or `packagedFonts({ install: true })` only for an
explicit page-wide font replacement. These register substitutes under Word family names.
They can hide native glyphs for scripts the substitutes do not cover.
To avoid another font request, pass already loaded bytes:
...
after parsing, with the family names that document declares. It calls the resolver
again when typing, formatting, or another edit introduces a new family. Selecting
a font for an empty paragraph also requests it.
...
passed as `resolvedFaces`, so providers can skip redundant downloads. Failed
families are attempted once per document load; reload the document to retry.
...
Both pass loaded bytes to the editor, which registers private aliases without
another request. The explicit `installDefaultFontFaces()` helper reuses supplied bytes.
If a face has no supplied bytes, that helper can load its URL through the browser's
`FontFace` loader, outside your `fetcher`. The same applies to
`packagedFonts({ install: true })`.
