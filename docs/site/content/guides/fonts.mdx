...
If you call `installDefaultFontFaces()` without bytes, it can fetch through the
browser's `FontFace` loader, outside your `fetcher`.
The legacy `packagedFonts({ install: true })` option follows the same registration path.
