...
const { document, fonts } = useDocxSource(url, { fonts: packagedFonts({ install: false }) });
```
...
const fonts = await defaultFonts({ install: false, families: ALL_WORD_DEFAULT_FAMILIES });
```
...
```ts
const fonts = packagedFonts({ install: false, allow: ['Calibri'] });
```
...
const { document, fonts } = useDocxSource(url, { fonts: () => defaultFonts({ install: false }) });
```
...
```ts
import { defaultFonts, packagedFonts } from '@docx-editor.dev/fonts';

const onDemandFonts = packagedFonts({ install: false });
const eagerFonts = await defaultFonts({ install: false });
```
...
const { document, fonts } = useDocxSource(url, {
  fonts: [brand, packagedFonts({ install: false })],
});
```
...
| Composition                | `useFonts(packagedFonts({ install: false }), googleFonts())` combines sources. |
...
With `install: false`, both loaders pass bytes to the editor for private registration.
Public registration, enabled by default, also reuses loaded bytes. If bytes are
unavailable, registration can use the browser's `FontFace` loader outside your
`fetcher`. `installDefaultFontFaces()` follows the same rule.
...
```ts
const fonts = useFonts(computed(() => packagedFonts({ install: false })));
```
