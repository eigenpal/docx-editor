...
written in. When it does, the editor extracts them on load, measures with them, and
registers them with the browser (`document.fonts`) so the rendered pages paint with the
same glyphs — registration is removed again when the document is replaced or the editor
is destroyed. No configuration, no assets, no network: a document that brings its own
fonts is Word-accurate out of the box, in metrics and in pixels.
