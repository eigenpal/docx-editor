...
<Callout>
`@docx-editor.dev/fonts` is not published to npm yet — it ships with the v2 line. The
API below is stable; the package is workspace-only until then.
</Callout>
...
position do not — which is why the sample above waits for fonts before the first mount.
