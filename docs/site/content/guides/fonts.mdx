...
### Update apps that use page-wide fonts
...
For on-demand loading, use `packagedFonts({ install: true })`.

For eager loading, register the loaded bytes:
...
Pass `fonts` to the editor's `fonts` option. Reusing these bytes avoids another
font request. Public registration affects the whole page and can hide native
glyphs for scripts the substitutes do not cover.
