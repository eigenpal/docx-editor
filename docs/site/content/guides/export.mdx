...
If a converter handler is missing, the menu shows an error that names the required package and configuration. The menu shows conversion failures and disables export actions while conversion runs. After a failure, select the format again to retry. A missing converter never falls back to another format.
