...
Select **File > Export > Markdown (.md)** or **File > Export > PDF (.pdf)**. Each export uses the editor's saved document bytes. Export does not change the document or its undo history.

## Try document export

Edit the sample, then download DOCX bytes or convert them to Markdown. Inspect the Markdown output before downloading it. The PDF example shows the required server integration.

<ExportGuideDemo />

## Install the converters

The editor packages do not install either converter. Install only the formats you enable. Opening the editor or its menu does not load a converter.
...
Configure only the conversion handlers your application uses. Pass them through `menu.exporters` on the packaged editor. For a composed editor, pass `exporters` to `DocxEditor.Menu` or `DocxEditorMenu`.
...
Use `popups.export` to replace progress and error feedback through the shared popup API. For examples, see [Customize export feedback](/docs/2.x/guides/customize-dialogs#customize-export-feedback).
...
The demos enable both export formats explicitly. Their frontend packages install Markdown; the shared server installs PDF.

The demos use `displayMode: 'proposed'` for both formats. Exports include tracked insertions and omit tracked deletions.
