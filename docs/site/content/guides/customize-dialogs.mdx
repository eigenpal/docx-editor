...
In the Page Setup, Paragraph Options, and Field Options parts, `asChild` gives your
element the wiring (`type`, `onClick`, `disabled`, `data-docx-part`) and none of the
packaged `docx-dialog__*` classes, so your own styles or utility classes apply as
written. To keep the packaged look and adjust it, pass `className` to the part
instead of `asChild`. Other popups still pass their own classes through `asChild`.
