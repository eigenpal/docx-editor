...
| `--doc-dialog-radius`      | The panel corner radius. Inputs and buttons keep `4px`. |
...
Every default rule has two-class specificity, such as `.docx-editor .docx-dialog__button`,
so it survives element resets like Tailwind preflight. To override a part, anchor
your selector with `.docx-editor` and style your replacement without `!important`:
...
A selector with one class, such as `.app-apply` alone, loses to the defaults.
