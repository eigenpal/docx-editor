...
combo box, date, or checkbox control. Without children it renders the packaged
arrangement: a list, a list with free-text entry, or a month calendar with a
**Today** button, from the same stylesheet classes the engine's own menus use. A
checkbox session applies its toggle at once and renders nothing.
...
dropdown, combo box, and checkbox sessions. This example keeps their default
controls, adds a class to the date grid, and replaces the Today button:
