...
Use `DocxEditorContentControlWidget` to customize content-control popups. Omit
children to keep the default UI, or pass parts to replace its contents. The root
handles placement, Escape, outside clicks, focus return, and Tab wrapping.
...
Check `session.kind` before rendering date-specific parts. Keep the default root
for other kinds. This example styles the grid and uses `asChild` to replace the
Today button's element while retaining its behavior.
...
Import `BrandedWidget` and register it on your editor:

```tsx
<DocxEditor document={source} popups={{ contentControlWidget: BrandedWidget }} />
```
...
`source` is your existing editor document. Import `DocxEditor` from your framework's
package. Parts accept `className`, `style`, `hidden`, and `asChild` and forward
`id`, `aria-*`, and `data-*`. With `asChild`, provide one element; custom components
must forward its props, events, and ref.
...
`Header` includes month and year selectors. `Footer` includes Cancel and Apply.
Keep `Error` in custom arrangements so users can see rejected writes.

### Add a custom action

Call `useContentControlWidget()` in a child of `Widget`. This React button commits
a date through the same validation as the packaged picker:
...
Place `<UseDepartureDate />` inside the date arrangement above. In Vue, import the
composable from `@docx-editor.dev/vue`; reactive state uses refs, which templates
unwrap automatically.

- `setValue(value)` updates the draft. `apply()` commits it; `apply(value)` commits
  an explicit value. A refused write returns `false`, keeps the popup open, and
  sets `refused`. `cancel()` closes without applying the draft.
- Dates use `YYYY-MM-DD`. `dateText`, `setDateText`, and `applyDateText` handle
  regional numeric entry using `session.locale`.
- Lists expose `items`. Pass an entry to `Item`, or call `apply(item.value)`.
  Gallery values name blocks in the document's glossary.
- Pictures expose `accept` and `replaceImage(fileOrBytes)`. Replacement returns
  `Promise<boolean>` and preserves the drawing's size.
- Checkbox values are `'true'` and `'false'`. A checkbox root without children
  toggles immediately; custom children must call `apply`.

### Style without replacing a renderer

Use `--doc-*` tokens for colors and `data-docx-part` attributes for individual
parts. These hooks work with the engine, React, and Vue renderers. Load your
stylesheet after the editor stylesheet.
...
Calendar cells expose `data-selected`, `data-today`, and `data-other-month`.
Popup roots expose `data-placement="top"` or `"bottom"`.

### Build a custom grid or root

Prefer packaged parts to retain keyboard behavior. For a custom grid, use
`calendar.days`, `focusDay`, and `calendarDateForKey` from
`@docx-editor.dev/core/editor`. Preserve grid roles, selected states, and one
focusable day. For a custom list, use `listId` and `listNavigation` to preserve
listbox relationships, typeahead, and roving focus.

For a separate popup root, call `observeContentControlPopup(panel, session.anchor)`
and run its returned cleanup on close. Use `contentControlPopupKeyDown` for Escape
and Tab wrapping. You must also handle outside clicks, focus return, and
`session.signal` cancellation. The packaged root handles these requirements.
