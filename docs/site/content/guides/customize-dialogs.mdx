...
With `asChild`, your element receives the wiring (`type`, `onClick`, `disabled`,
`data-docx-part`) and none of the packaged `docx-dialog__*` classes, so your own
styles or utility classes apply as written. To keep the packaged look and adjust
it, pass `className` to the part instead of `asChild`.
...
so a dark editor gets a dark dialog. The default look follows Word and Google Docs:
a white panel, bordered inputs, a filled primary button, and an outlined secondary one.
...
| `--doc-dialog-control-radius` | The corner radius of inputs, selects, and buttons.      |
| `--doc-dialog-padding`        | The inline inset of the header, body, and footer.       |
...
- **Adjust the packaged part.** Pass `className` to the part. Every default rule
  has two-class specificity, such as `.docx-editor .docx-dialog__button`, so it
  survives element resets like Tailwind preflight. Anchor your selector with
  `.docx-editor` and style it without `!important`. A selector with one class,
  such as `.app-rounded` alone, loses to the defaults.
- **Bring your own element.** Pass `asChild`. The packaged classes stay off your
  element, so a single class or utility classes style it as written.

```css
/* className="app-rounded" on the packaged Apply part */
