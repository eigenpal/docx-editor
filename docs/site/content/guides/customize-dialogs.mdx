...
The editor updates the same component when progress changes to an error. A later export starts a separate component instance. Successful downloads close the popup automatically. Dismissing feedback leaves conversion running and keeps export actions disabled until it finishes. If that conversion fails, the error opens the popup again.
