...
replaces the content of the control's first paragraph and keeps that paragraph's properties.
A checkbox toggle rewrites only the run that shows the state glyph. If the control's content
holds no paragraph and no run, such as foreign markup, the write is refused instead of
