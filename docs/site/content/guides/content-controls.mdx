...
Each dropdown, combo box, and date control shows a button past its right edge,
the way Word attaches its tab. The button scales with the page zoom. A checkbox
control's button covers its glyph. Select the button to toggle a checkbox, open a
dropdown menu, or open a date picker. The menu opens under the control's left
...
for dropdown, combo box, date, and checkbox entry. Omitted entries retain the
engine's own menus. A configured `contentControlWidget` renders
...
exposes the draft state. A checkbox press also reaches this renderer as a session
with `kind: 'checkbox'`; the packaged renderer applies the toggle at once, and a
renderer with its own children decides. See
