...
- **`@docx-editor.dev/agents`** wraps this same engine in a batching object model: reach a paragraph or a search match, queue edits against it, and one `sync()` applies them atomically. Use it when the transformation is described against document objects rather than against the raw model — including when a model decides what to change. See [Automation](/docs/1.x/agents).
...
- [Automation](/docs/1.x/agents) for the object-model alternative
