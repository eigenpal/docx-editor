...
description: 'Parse, inspect, mutate, and serialize DOCX files in Node.js or workers with @docx-editor.dev/core/headless. No DOM and no editor UI required.'
category: 'Guides'
seoTitle: 'Headless DOCX processing'
---

`@docx-editor.dev/core/headless` is the document engine without the editor UI: the OOXML parser, document model, mutation helpers, and serializer. It runs in Node.js and web workers. Nothing in it touches the DOM.
...
```ts
import { createDocumentWithText, createDocx } from "@docx-editor.dev/core/headless";
...
```ts
import { detectVariables, parseDocx, processTemplate } from "@docx-editor.dev/core/headless";
...
## When to use this vs the automation object model
...
- **`@docx-editor.dev/editor-api`** wraps this same engine in a batching object model: reach a paragraph or a search match, queue edits against it, and one `sync()` applies them atomically. Use it when the transformation is described against document objects rather than against the raw model — including when a model decides what to change. See [DocxEditor API](/docs/1.x/editor-api).

A pipeline can fill a template headlessly, then pass the bytes to `DocxEditor.createServer` for object-model edits.
...
- [DocxEditor API](/docs/1.x/editor-api) for the object-model alternative
