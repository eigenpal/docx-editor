...
For a complete React app and mock server, see [DOCX document refresh example](/examples/document-refresh). The editor does not watch a file path or URL. Your application receives updates and calls `refresh.applyUpdate()`.
...
Highlights are opt-in. Call `highlightChanges()` after a successful `applyUpdate()` when your interface needs them. These calls change presentation only:
