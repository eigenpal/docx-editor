...
For a complete React app and mock server, see [DOCX document refresh example](/examples/document-refresh). The editor does not watch a file path or URL. Your application receives updates and calls `refresh.apply()`.

## API overview

Create one controller with `createDocumentRefresh(editor)`. It provides these methods:

| Method | Purpose |
| --- | --- |
| `capture()` | Capture the current DOCX bytes and revision before starting a server job. Returns a submission. |
| `apply(update)` | Accept a complete DOCX using that submission and a processor-assigned sequence. Returns a success or refusal result. |
...
The update flow is `capture()` → your server job → `apply()` → `finish()`. Call presentation methods after a successful `apply()`. Scroll preservation is automatic; highlighting and navigation are opt-in.

Try the [interactive effects preview](#try-the-effects), then follow [Accept an updated file](#accept-an-updated-file).
...
Select an effect, then choose **Run effect**. Scroll inside the sample document to test scroll preservation. Highlight buttons never change the sample text. The preview shows the corresponding API call below the document.
...
The [DOCX document refresh example](/examples/document-refresh) includes a mock server and a React client. Run `bun run dev:refresh` after the setup steps in its README. Try scroll preservation, navigation with temporary highlights, and refusal after a local edit.
