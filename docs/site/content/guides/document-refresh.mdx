...
Choose one of four examples, then select **Run preview**. Each example shows one behavior and includes its API source code. Expand **Source code** to inspect the calls. Select **Replay** to run the example again.
...
The highlight fades in, remains visible, and fades out automatically. Both fades use `animation.durationMs` unless you set `animation.exitDurationMs`. Use this sequence after an explicit user action, because it changes the scroll position. Choose **Scroll and fade** in the preview to try it.
