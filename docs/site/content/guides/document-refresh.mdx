...
Pass presentation settings directly to `highlightChanges()`. Each call starts from the defaults, so reuse one options object for consistent styling. `RefreshHighlightOptions` extends `AnchorHighlightOptions`, so you can use the same options object for anchor highlights. For more information, see [Highlight a paragraph reference](/docs/2.x/guides/navigation#highlight-a-paragraph-reference).

For example, configure the highlight color, duration, and animation:
