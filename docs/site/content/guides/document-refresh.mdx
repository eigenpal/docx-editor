...
Pass presentation settings directly to `highlightChanges()`. Each call starts from the defaults, so reuse one options object for consistent styling. `RefreshHighlightOptions` extends `AnchorHighlightOptions`, so the same object also styles [anchor highlights](/docs/2.x/core#highlight-an-external-paragraph-reference):
