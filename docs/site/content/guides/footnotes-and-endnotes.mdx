...
A reference inside a table keeps its footnote with the table row that holds it. Later rows move to the next page to make room. If the note cannot fit below its row, the row moves to the next page with the note. References in header rows, vertically merged cells, positioned tables, and rows that keep with the next row reserve room below the whole table instead.
