...
When a reference line moves to the next page, the lines above it stay on the current page if they fit. Widow and orphan control, keep lines together, and keep with next decide which of those lines stay.

The note area can use the space below the last body line's text when that line uses `auto` or `atLeast` line spacing. A line that carries a reference always keeps its full line height above the note area.
