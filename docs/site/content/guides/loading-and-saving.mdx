...
Use `createDocumentRefresh(editor)` when a server returns an updated DOCX file. The controller preserves the editor instance and scroll position. Each accepted file resets selection and undo history. Results after local edits and collaborative sessions are refused.
