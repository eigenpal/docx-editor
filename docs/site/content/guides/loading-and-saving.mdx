...
`save()` on the ref resolves `null` when no editor is mounted, so guard the result.
...
Locked fields and unsafe result structures keep their saved values. View mode,
read-only sessions, and collaborative sessions do not rewrite field results.

See [Fields and cross-references](/docs/2.x/guides/fields) for supported switches
and other limits.
