...
If you already hold a parsed `Document` tree from `@docx-editor.dev/core`, pass it through the `document` prop instead and skip the parser.
...
For local crash recovery rather than server persistence, the React adapter ships `useAutoSave` in `@docx-editor.dev/react/hooks`: it persists the parsed `Document` to localStorage on an interval and exposes recovery data (`hasRecoveryData`, `acceptRecovery`) after a reload.
