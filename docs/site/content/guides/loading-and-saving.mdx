...
To open the editor on an empty page, pass `'blank'`. It is Word's blank template, with the same Calibri 11pt defaults a new document in Word has. It also carries Word's built-in style gallery: Heading 1 through Heading 9, Title, Subtitle, Quote, No Spacing, and List Paragraph.
...
`useDocxSource()` fetches the bytes and tracks the request, so a changing URL never applies a stale response.
...
`useDocxSource()` takes a URL string, a `URL`, a `Uint8Array`, or an `ArrayBuffer`. For a picked file, read the bytes first, as this sample does.
