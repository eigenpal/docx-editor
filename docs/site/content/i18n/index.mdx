...
The editor's `locale` prop selects regional date input and engine-generated labels;
`i18n` supplies UI strings. For example, use `locale="pl-PL"` with `i18n={pl}` for
Polish date input and Polish chrome, or keep English chrome while using `en-GB`
date input. Full regional tags are preserved independently of catalogue fallback.
See [form fields](/docs/2.x/guides/fields) for supported date syntax and defaults.
...
In Vue, use `locale="pl-PL"` and `:i18n="pl"` on `<DocxEditor>`.
