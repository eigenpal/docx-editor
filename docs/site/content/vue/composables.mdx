...
shows that as indeterminate. A number field has no such state, so it opens on one
of the values and stays there until you change it. Omit a field to leave it as
