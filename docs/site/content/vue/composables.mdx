...
immediately; one gesture takes one Undo press. The composable retains the
handle across component updates and disposes it on unmount or editor replacement. Use `range`, `repeat`, `keyboard`, or
`native-color` to select the event policy.

The following native color control reads the selected color and reports refusals.
...
The native `change` event ends the color commitment. The input handler only
applies values. `useEditorCommand().execute(options)` accepts group options and
keeps its boolean result. Its nullary event-handler form still works.
...
for native color limitations and a deterministic range example.
...
`execute(value, options?)` returns `ExecResult`. On refusal, use `code` in your
application logic and display `reason`. On success, `changed` distinguishes a
document edit from a caret-only formatting change. `result.history?.kind`
reports `started`, `extended`, `split`, or `none`. An intervening edit can split
a gesture without refusing its formatting.

The native color `change` event means value commitment, not popup closure.
If a browser sends no terminal commitment, the next activation, Escape, blur,
or unmount ends the gesture. For portable drag boundaries, use a range control.
...
`value` stays synchronized with undo and selection changes; null means mixed or
unavailable. Image commands refuse history groups.
