...
Returns the draft state of the enclosing content-control value pop-up: the
session, `value`, `setValue`, `isEnabled`, `refused`, `apply`, `cancel`, and the
calendar members `calendar`, `previousMonth`, `nextMonth`, `selectDay`,
`selectToday`, `focusIso`, and `focusDay`. `showMonth(year, month)` jumps to a
zero-based month and a year from 100 through 9999. `dateText`, `setDateText`, and
`applyDateText` support regional numeric entry while `value` stays ISO.
`listId` connects a combo input to its list. `listNavigation.keyDown` supplies
shared typeahead and roving focus. `accept` and `replaceImage(fileOrBytes)` serve
picture sessions. The packaged root calls it automatically.
Reactive members are refs; `listId` and `listNavigation` are stable objects or values. Call it inside a child
of `DocxEditorContentControlWidget`. For the parts it pairs with, see
...
`useHistoryGroup({ kind })` returns an element callback `ref` and `options()`.
Place the control inside `DocxEditor.Root`. Each input updates the document
immediately, and one undo action restores the state before the gesture. The
composable retains the handle across component updates. It disposes the handle
on unmount or editor replacement. Use `range`, `repeat`, `keyboard`, or
`native-color` to select the event policy.

The following native color control reads the selected color and reports rejected commands.
`null` means mixed or unavailable. Undo and redo update `value` automatically.
Colors use six hexadecimal digits without `#`.
...
The native `change` event ends the gesture. The `input` handler applies values. `useEditorCommand().execute(options)` accepts group options and
keeps its boolean result. You can also use `execute` as an event handler without passing options.
Use `useEditorEvent('historyDiagnostic', handler)` for optional diagnostics.
See [history grouping and boundaries](/docs/2.x/core#group-one-gesture-into-one-undo-step)
for browser differences and a complete range control example.

For a font-size slider, use `useEditorValueCommand('font.size')` and
`useHistoryGroup({ kind: 'range' })`. Set `type="range"` and pass
`Number((event.target as HTMLInputElement).value)` to `execute`. Values use
half-points: `24` means 12 pt. Pointer release and keyup end each gesture,
including release outside the control. Keep your displayed value bound to the
returned `value` ref.

`execute(value, options?)` returns `ExecResult`. If `ok` is `false`, use `code`
in application logic and display `reason`. If `ok` is `true`, `changed` tells
you whether the document changed. Formatting at a caret can set the format for
subsequent typing without changing the document. `result.history?.kind`
reports `started`, `extended`, `split`, or `none`. An intervening edit can split
a gesture while still applying the formatting.

For native color inputs, `change` signals a committed value. It does not
guarantee that the color picker closed. If the browser sends no `change` event,
the next activation, Escape, blur, or unmount ends the gesture. If you need
consistent drag boundaries across browsers, use a range control.
A `repeat` binding groups writes from your held-button timer; your app must
start and stop that timer itself.

## useEditorValueCommand

Returns typed state and actions for `text.color`, `font.size`, `font.family`,
`text.highlight`, `styles.style`, `list.lineSpacing`, and image value slots.
`execute(value, options?)` returns `ExecResult`. Font size uses half-points.
`value` stays synchronized with undo and selection changes. A `null` value means mixed or
unavailable. Image commands reject the `historyGroup` option.
...
Call the composable in a child component of the matching dialog. Each returns `values`,
