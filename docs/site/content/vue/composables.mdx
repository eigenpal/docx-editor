...
Pass an externally owned collaboration session to `useCollaborationStatus`.
The composable returns a read-only `status` ref:

```ts
import { useCollaborationStatus } from '@docx-editor.dev/vue';

const { status } = useCollaborationStatus(session);
```

The ref contains `inactive` when the session value is `null`.
The experimental Yjs proof supports text edits in existing body paragraphs only.
...
[font guide](/docs/2.x/guides/fonts).
