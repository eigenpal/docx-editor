...
Call `useContentControlWidget()` in a child of `DocxEditorContentControlWidget`
to read its draft and commit values. Reactive members are refs; templates unwrap
them automatically. `listId` and `listNavigation` are stable values.
...
for registration, parts, and custom actions.
