...
`:fonts` supplies the font bytes the engine measures with and `:font-families` constrains the picker; the full rules are under [React props → Fonts](/docs/1.x/react/props#fonts) and apply unchanged.
...
import type { FontConfigurationFragment } from '@docx-editor.dev/vue';

const fonts = ref<FontConfigurationFragment>();
onMounted(async () => {
...
<template>
  <DocxEditor
    ...
    @font-error="(e) => report(e.code)"
  />
</template>
```

Keep the reference stable — `:fonts` is sampled at mount and an identity change remounts the editor. Per-face failures surface through `@font-error`. See the [Fonts and measurement](/docs/1.x/guides/fonts) guide for the three font sources and how they compose.
...
There is no automation prop or slot on either adapter. `@docx-editor.dev/agents` is imported directly; in Vue that means the server entry over bytes, because the `/browser` entry needs an editor instance the Vue ref does not expose yet. The round trip is in [Vue examples](/docs/1.x/vue/examples).
