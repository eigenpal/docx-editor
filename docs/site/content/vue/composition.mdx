...
Choose toolbar controls, placement, labels, spacing, and a visual style. Add optional document controls. The preview runs the Vue adapter and includes the selected components and CSS.

These example layouts use the composition API; they are not packaged toolbar presets. The demo shows a subset of the available components and options. Use your own Vue components for toolbars, menus, and panels. Connect them with [Vue composables](/docs/2.x/vue/composables), or combine them with packaged parts.
...
Page Setup, Paragraph Options, and legacy text Field Options expose replaceable parts and draft contexts. Import `definePopup` from `@docx-editor.dev/vue`. Set `popups.pageSetup` to `definePopup(MyPopup)` on the editor or `Root`. Use `popups.export` and `DocxEditor.ExportDialog` to customize File menu export feedback. Use `popups.print` and `DocxEditor.PrintDialog` to customize File menu print feedback. For examples, see [Customize popups](/docs/2.x/guides/customize-dialogs).
