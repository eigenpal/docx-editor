...
Page Setup, Paragraph Options, and legacy text Field Options expose replaceable
parts and draft contexts. Import `definePopup` from `@docx-editor.dev/vue`.
Set `popups.pageSetup` to `definePopup(MyPopup)` on the editor or `Root`.
