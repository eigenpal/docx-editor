...
The main React hook from `@docx-editor.dev/agents/react`. It creates the bridge to the editor and returns the tool schemas, executor, and context snapshot.
...
The Vue twin, from `@docx-editor.dev/agents/vue`:

```ts
import { ref } from 'vue';
import { useAgentBridge, type EditorRefLike } from '@docx-editor.dev/agents/vue';
...
For hosts that are neither React nor Vue, build the bridge directly from `@docx-editor.dev/agents/bridge`:

```ts
import { createEditorBridge, type EditorRefLike } from '@docx-editor.dev/agents/bridge';
import { executeToolCall, getToolSchemas } from '@docx-editor.dev/agents';
...
The panel shell is configurable through these options; render your own chat UI inside `render`. In Vue the `agentPanel` prop does not exist yet (React-only per the parity contract); mount the `AgentPanel` component from `@docx-editor.dev/agents/vue` next to the editor instead.

## Chat UI components

Optional components for the inside of the panel. React: `@docx-editor.dev/agents/react`. Vue: `@docx-editor.dev/agents/vue`.
...
If you drive the chat with the Vercel AI SDK, `toAgentMessages(chat.messages, chat.status)` from `@docx-editor.dev/agents/ai-sdk/react` (or `/ai-sdk/vue`) converts `useChat`'s `UIMessage[]` into the `AgentMessage[]` shape these components render. You can also render components from another chat UI library or your own design system.
