...
The following demo creates a structured DOCX. After the draft finishes, choose
**Redline for clarity** to see tracked changes a reader can accept or reject.

<AgentChatDemo agent="writer" />
...
`@docx-editor.dev/editor-api` and `@docx-editor.dev/pro` use the
[EigenPal Pro Evaluation License 1.0](https://github.com/eigenpal/docx-editor/blob/main/packages/pro/LICENSE.md).
Internal evaluation is free. Production use needs a written commercial
agreement.
...
<Callout>
  Do not put `•` or `1.` in paragraph text. Native list formatting adds those
  markers. Literal prefixes show up twice in Word.
</Callout>
...
<Callout>
  Wrap the placeholder, not the label. Wrap `[Effective Date]`. Do not wrap
  `Date of Birth`.
</Callout>
