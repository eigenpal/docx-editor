...
Use `createDocumentRefresh(editor)` when your agent or external processor returns a complete updated file. Call `capture()` before processing, then pass the returned DOCX bytes, submission, and sequence number to `refresh.apply()`.
...
See [Document refresh API](/docs/2.x/guides/document-refresh) for options and failure handling.
