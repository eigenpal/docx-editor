...
Both packages export the same `createFontSource` and `defineFontResolver` helpers. For font registration, see [Configure PDF fonts](/docs/2.x/export/pdf/fonts). These settings align common controls; they do not guarantee identical pagination for every document. Missing glyphs, fallback faces, and unsupported features can still affect output.
