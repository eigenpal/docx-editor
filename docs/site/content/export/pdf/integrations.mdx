...
The exporter disposes its document session after each call, including failed calls. For repeated exports or both formats from one layout, use [a reusable PDF session](/docs/2.x/export/pdf/markdown-contract#reuse-one-session).
