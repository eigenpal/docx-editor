...
Start with the [installation requirements and package availability](/docs/2.x/export/markdown#get-started).

## Node.js and serverless functions

Use a Node.js runtime that allows WebAssembly and retain the packages' font and WASM assets in the deployed artifact. Edge runtimes are unsupported. The default font sources are bundled; remote fallback is opt-in through `fallbackFonts`.
