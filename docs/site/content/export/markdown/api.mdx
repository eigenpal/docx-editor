...
Fonts and revision visibility affect page breaks. The default uses bundled substitutes, such as Carlito for Calibri. Supply the author's fonts when licensed to do so, inspect `fontResolution`, and validate representative documents against Word if readers need matching page references. `fontPolicy: 'strict'` checks font resolution; it does not guarantee Word-identical pagination. See [Configure fonts for accurate layout](/docs/2.x/export/markdown#configure-fonts-for-accurate-layout) for the role of fallback fonts and a Google Fonts example.
...
JSON and ZIP helpers come from the main package. Portable bundles require generated relative image URLs. See [image delivery examples](https://github.com/eigenpal/docx-editor/blob/main/packages/docx-to-markdown/docs/images.md) for storage callbacks and renderer setup.
