...
Compression applies only between adjacent punctuation. Paragraph and manual-line
boundaries retain their advances. Colons can compress before opening brackets,
bounded by the following bracket's available bearing. Other centered punctuation,
ordinary text, authored spaces, outlined text, decorated text, and tracked changes
...
Kana uses deterministic advance reductions. Soft-wrap edge compression and
font-specific optical compression are not modeled.

The document's `w:themeFontLang/@w:eastAsia` selects its language-specific theme face.
When that setting is absent and the East Asian theme face is empty, inherited
`w:lang/@w:eastAsia` selects the supplemental face. Chinese, Japanese, and Korean use named
CJK defaults when neither the theme nor the run supplies a face. Font origins receive
these candidates before shaping. Supply suitable CJK fonts for exact glyph metrics.
