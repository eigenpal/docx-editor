...
A floating object in a table cell can turn off the **Layout in table cell** option. Word honors that setting only in compatibility mode 14 or earlier, or when the document declares no mode. The object is then placed against the page, and the cell text runs through it without wrapping. Top-and-bottom wrapping still moves the cell text below the object, because Word moves the whole table below it. In compatibility mode 15 and later, Word ignores the setting and keeps the object in its cell, and so does the editor.
