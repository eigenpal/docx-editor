...
Vertical cell merges that cover the same rows share those rows without adding their heights twice. In fixed-height rows, each merged cell displays the content that fits the complete span.
