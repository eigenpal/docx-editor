...
Passages of a quarter inch or less remain empty. Captions and headings clear these narrow gaps.
...
Shared-bearing compression applies between adjacent punctuation. Paragraph and manual-line
boundaries retain their advances. Colons can compress before opening brackets,
...
Narrow plain LTR East Asian paragraphs can also use measured punctuation bearings to fit the next ordinary glyph
and its protected punctuation group. Fitting uses measured glyph bounds and preserves source positions.
Mixed Latin text, fields, decorations, revisions, RTL text, float passages,
...
CJK defaults when neither the theme nor the run supplies a face. Word can supply different
missing-language defaults from its installation language and keyboards. Font origins receive
these candidates before shaping. Supply suitable fonts for exact glyph metrics.
