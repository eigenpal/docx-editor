...
If a paragraph starts with a manual page break and has text after the break, that text starts on the next page. This also applies when the current page has no room for another line. The position before the break stays at the bottom of the current page. In compatibility mode 15 and later, the space before of the paragraph goes to the text on the next page. In earlier modes, and when the document declares no mode, that text has no space before. In a numbered or bulleted paragraph, the number or bullet goes to the next page with the text.

This rule also applies to a paragraph with borders or shading. Lines that hold only page breaks draw no border. The text after the breaks opens its own border box on the next page, with the top border of the paragraph. That box does not join the border group of the paragraph above, so the paragraph above closes its box with its bottom border. Shading fills each break line where it sits, up to the page edge.

This rule does not apply to some paragraphs. A paragraph with only a page break keeps the ordinary rule. A paragraph that anchors a floating table, text frame, or drawing also keeps it. If such a paragraph follows a full page, its break line moves to a new page, and its text starts one page later.
