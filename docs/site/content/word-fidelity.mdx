...
A paragraph with `w:keepNext` stays with the opening lines that the following paragraph must keep together. With widow control, a following three-line paragraph stays whole. `w:keepLines` keeps the entire following paragraph together when the group fits on a page.
...
Text-anchored tables taller than a content page retain row pagination. A text-anchored table that covers the text column also breaks across pages when the rest of the page cannot hold it. It breaks only when its first rows fit on that page. The first rows are any header rows and the first body row. The remaining rows continue at the top of the next page, and the following text starts below the last row. This continuation applies to single-column sections with one table per anchor. Anchors with page or column breaks, or space before them, keep whole-table placement. Tables with text distances above or below them also keep whole-table placement. If the first rows do not fit, the table moves to the next page with its following paragraph. Tables that fit a full page move together when `w:doNotBreakWrappedTables` is enabled. Text-anchored tables marked no-overlap or using vertical alignment also retain their normal flow behavior. Tables whose cells can wrap around earlier floating objects also retain row pagination. Simple tables can share a terminal empty paragraph without shrinking that paragraph or adding a blank page. Floating-table properties survive save, but have no editing interface.
...
| Drawing shapes | Renders solid rectangles, ellipses, lines, bounded polygons, and groups, with outlines drawn at full width. A group can include one unrotated picture below its shapes. Other shapes use a placeholder. | No | Preserved |
...
Manual page breaks (`w:br w:type="page"`) inside table cells do not add lines, line height, or pages. This applies to breaks at the start, middle, or end of a cell paragraph. The text on each side of a break stays on the same line. Line wrapping, tab alignment, and text wrap around floating pictures treat the text as if the break were absent. The document retains these breaks for editing and saving. Manual line breaks still start a new line inside a cell.

Known single-byte Symbol and Wingdings bullets use Unicode fallback when their font is unavailable. The editor preserves the original numbering definition when you save.

Bottom-to-top cell text does not make its row taller. The text wraps at the final row height, or along the merged cell height for a merge head, and is clipped at the cell width.

Vertical cell merges that cover the same rows share those rows without adding their heights twice. In fixed-height rows, a merged cell whose content fits the complete span displays all of it. If the content does not fit, the first row of the merge clips it.
...
If a paragraph starts with a manual page break and has text after the break, that text starts on the next page. This also applies when the current page has no room for another line. The position before the break stays at the bottom of the current page. The text on the next page gets the space before of the paragraph, in every compatibility mode. That space overlaps the space after of the paragraph above, as it does between any two paragraphs, so the text gets only the part that is larger. The space after of the text overlaps the space before of the next paragraph in the same way. In a numbered or bulleted paragraph, the number or bullet goes to the next page with the text.

This rule also applies to a paragraph with borders or shading. Lines that hold only page breaks draw no border. The text after the breaks opens its own border box on the next page, with the top border of the paragraph. That box does not join the border group of the paragraph above, so the paragraph above closes its box with its bottom border. This happens only when the displayed paragraph starts with the break. A break in hidden text, or in a tracked change that the current view does not show, keeps the paragraph in the group. Shading fills each break line where it sits, up to the page edge.

This rule does not apply to some paragraphs. A paragraph with only a page break, or with only hidden content after the break, keeps the ordinary rule. A paragraph that anchors a floating table, text frame, or drawing also keeps it. If such a paragraph follows a full page, its break line moves to a new page, and its text starts one page later.

An empty paragraph that ends a section ignores its own page break before, from direct formatting or its style. It stays after the section's content, so it does not add a blank page. The next section's break type controls where that section starts. This rule does not apply when nothing comes before the paragraph in its section. It also does not apply when the paragraph holds text, a tab, hidden text, or a field. Bookmarks, proofing marks, permission markers, comment range markers, and empty text runs do not prevent this rule.

Saved results of fields inside another field's instruction do not add text, line breaks, or links. The outer field keeps its normal display behavior. Fields inside a saved result still display.

Named character styles set the font properties of empty paragraph marks and numbering markers. Direct mark properties override those styles. A mark's character style also sizes the empty line after a trailing line break. It does not enlarge a line that contains text or an inline picture. A direct size on the mark does not enlarge a line that contains text, an inline picture, or an equation either. A line with only inline pictures takes the text height of the runs that hold them, not of the paragraph style or the mark. The last line of a paragraph keeps the full-size height of its superscript or subscript text, and a larger paragraph style size does not make that line taller. When you type in an empty paragraph, the new text takes the character style of the paragraph mark.
...
A `w:br` or `w:cr` directly inside a paragraph or hyperlink, outside a run, is kept as a generic node and saved unchanged in place. The paragraph and its runs still render and stay editable. The misplaced break does not start a new line or page. A `w:tab` or `w:t` directly inside a paragraph, outside a run, still makes the whole paragraph unknown content. That paragraph and its text are kept on save, but the paragraph does not render and you cannot edit it.
