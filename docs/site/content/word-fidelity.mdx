...
If a paragraph starts with a manual page break and has text after the break, that text starts on the next page. This also applies when the current page has no room for another line. The position before the break stays at the bottom of the current page. In a numbered or bulleted paragraph, the number or bullet goes to the next page with the text.

This rule does not apply to some paragraphs. A paragraph with only a page break, and a paragraph with borders or shading, keep the ordinary rule. A paragraph that anchors a floating table, text frame, or drawing also keeps it. If such a paragraph follows a full page, its break line moves to a new page, and its text starts one page later.
