...
- **Automation is host-controlled.** [Document automation](/docs/1.x/agents) ships no network transport and no model integration: the browser entry drives an editor already in the page and the DOCX never leaves it, and the server entry runs wherever you host it. What reaches a model is whatever your own code sends.
