...
| Drawing shapes | Renders solid rectangles, ellipses, bounded polygons, and groups. A group can include one unrotated picture below its shapes. Other shapes use a placeholder. | No | Preserved |
...
Rows with bottom-to-top cell text use the text length when they have no fixed height. If a minimum height or another cell makes the row taller, text alignment still uses the measured text length.
...
If a plain paragraph starts with a manual page break and has text after the break, that text starts on the next page. This also applies when the current page has no room for another line. The position before the break stays at the bottom of the current page.

This rule does not apply to some paragraphs. A paragraph with only a page break, and a paragraph with list numbering, borders, or shading, keep the ordinary rule. A paragraph that anchors a floating table, text frame, or drawing also keeps it. If such a paragraph follows a full page, its break line moves to a new page, and its text starts one page later.
...
A footnote area can use the space below the text of the last body line on its page. This applies when that line uses `auto` or `atLeast` line spacing. A line that carries a footnote reference keeps its full line height above the notes. In multi-column sections, the notes can use the blank space below the text of a reference line. If a reference line moves to the next page with its note, the lines above it stay on the current page when they fit. Widow and orphan control and keep lines together decide which of those lines stay. A keep-with-next group that holds the reference line and fits on one page moves as one piece. This applies to single-column pages without text-wrapping pictures, text frames, or floating tables.

Some edits within keep-with-next chains can retain extra blank space or an extra page. Difficult multi-column layouts can still separate notes from references.
