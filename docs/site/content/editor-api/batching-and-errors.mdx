...
Check `isNullObject` after sync. Do not read other properties on a null object. A non-null accessor such as `getFirst()` fails with `ItemNotFound` when its collection is empty. Collections expose different accessors; check the [API member directory](/docs/2.x/editor-api/reference).
...
`ClientResult<T>` exposes `isLoaded` and `value`. Read `value` after the operation's `context.sync()` completes; reading it earlier throws `ValueNotLoaded`. For example, `RevisionCollection.resolve()` returns a `ClientResult<RevisionBatchResult>` with resolved and skipped decisions and a remaining count. See [Resolve a batch of changes](/docs/2.x/editor-api/revisions#resolve-a-batch-of-changes).
