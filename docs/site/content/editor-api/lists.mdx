...
Paragraph margins and line height come from document defaults, the paragraph style and its `basedOn` ancestors, and direct formatting. Inside tables, table styles can also supply paragraph formatting. Direct formatting overrides the inherited attributes it specifies; omitted attributes keep their inherited values. Numbering definitions supply the marker and list indentation.
...
The editor follows the document relationship when style definitions use a different package filename. Line-unit paragraph margins (`w:beforeLines` and `w:afterLines`) are not yet resolved. Numbering-level paragraph properties other than indentation also need further compatibility work. These limits apply independently of Enter preserving the authored style references.
