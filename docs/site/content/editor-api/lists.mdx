...
List items inherit paragraph spacing from document defaults, paragraph styles, and direct formatting. Enter preserves these settings. When preceding items at the same list level have one blank paragraph between them, Enter also inserts an unnumbered separator before the new item. Undo removes both inserted paragraph breaks together. Tracked editing records both breaks for acceptance or rejection. Automatic spacing stays tight between adjacent items in the same list, including nested levels. The list keeps its outer automatic margins, except before a list at section start.

Explicit spacing still applies between items unless the paragraph enables **Don’t add space between paragraphs of the same style**. Blank paragraphs and line breaks add height independently of these spacing settings.
