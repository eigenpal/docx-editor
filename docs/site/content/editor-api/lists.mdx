...
List items inherit paragraph spacing from document defaults, paragraph styles, and direct formatting. Enter preserves these settings. Automatic spacing stays tight between adjacent items in the same list, including nested levels. The list keeps its outer automatic margins, except before a list at section start.
