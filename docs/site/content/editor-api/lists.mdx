...
Paragraph margins and line height come from document defaults, the paragraph style and its `basedOn` ancestors, and direct formatting. Inside tables, table styles can also supply paragraph formatting. Direct formatting overrides the inherited attributes it specifies; omitted attributes keep their inherited values. Numbering definitions also supply paragraph spacing, alignment, tabs, and borders. When a paragraph states its own numbering properties, even only a list level, its level properties apply after the paragraph style; direct paragraph properties still win. Otherwise, the level properties apply after the base styles of the nearest style that states numbering properties. That style, the styles based on it, and direct paragraph properties override the level. Indents follow the same order.

Enter preserves that inheritance instead of copying resolved spacing into the new paragraph. A style's **Style for following paragraph** setting can start a different style; entering that style drops the old direct numbering. Shift+Enter creates a line break within the existing paragraph.

Word's AutoFormat list continuation is separate from paragraph spacing. When the preceding item belongs to the same list and level, with exactly one unnumbered blank paragraph between them, Enter inserts an unnumbered separator before the new item. Spaces or tabs in the blank paragraph do not change this behavior. It applies to bullets, numbered lists, and Enter replacing text within an item. A different list, different level, or a gap only after the current item does not establish this pattern. Content-control boundaries preserve the pattern within the same flow. A paragraph containing a shape, field, or reference does not count as blank.

The separator uses the document's **List Paragraph** style and the direct typing format, even when the items use a custom style. If that built-in style is missing, the editor creates it based on the document's default paragraph style. The items keep their own styles. Undo restores the paragraphs and any newly created style together; redo restores the caret on the new item. Tracked editing records both inserted breaks for acceptance or rejection.

Automatic spacing stays tight between adjacent items in the same list, including nested levels. The list keeps its outer automatic margins, except before a list at section start. Explicit spacing still applies between items unless the paragraph enables **Don’t add space between paragraphs of the same style**. Blank paragraphs and line breaks add height independently of these settings.

The editor follows the document relationship when style definitions use a different package filename. Line-unit paragraph margins (`w:beforeLines` and `w:afterLines`) use a fixed 12-point unit without a document grid. An active section grid supplies its line pitch instead. Line-unit values take precedence over point values, including inherited values; automatic spacing takes precedence over both. Contextual spacing applies in body text and table cells, including paragraphs using the implicit default style.
