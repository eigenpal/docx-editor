...
The API edits inline PNG and JPEG pictures. Use `Body.inlinePictures` or `Range.inlinePictures` to discover pictures. Floating shapes, picture bullets, and a general `Shape` object are outside this subset.

Create a `runtime` with [Runtime and setup](/docs/2.x/editor-api/runtime). Use `context` and document proxies inside `runtime.run()`.

## Insert image bytes

`Range.insertInlinePictureFromBase64(base64EncodedImage, location)` accepts `Replace`, `Start`, `End`, `Before`, or `After`. Pass the base64 image bytes, and sync before configuring the returned `InlinePicture`.
...
`InlinePictureCollection` exposes `items`, `getFirst()`, `getLast()`, `getFirstOrNullObject()`, and `getLastOrNullObject()`. Check `isNullObject` after sync when using a nullable accessor. A range collection includes pictures fully contained within that range.
...
See [Text and ranges](/docs/2.x/editor-api/text-and-ranges), [Batching, loading, and errors](/docs/2.x/editor-api/batching-and-errors), and [API member directory](/docs/2.x/editor-api/reference).
