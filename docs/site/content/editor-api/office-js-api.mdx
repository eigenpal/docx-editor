...
`Comment.reply()` uses the explicit runtime author: pass `{ author: 'Demo Reviewer' }` to either
`createBrowser` or `createServer`. A missing or blank author refuses with `NotSupported` before the
write is queued. In a browser, the Pro review module and a writable editing mode are also required.
There is deliberately no static comment-write capability because those conditions can change; a
typed refusal from the call or `sync()` is authoritative.
...
| Creating comments                                          | Comments can be read, replied to, resolved, and deleted. This release does not create them.                                       |
