...
Every returned item is individually actionable, so collection membership is the acceptability
signal:
...
Structural revisions are preserved but omitted from `items` because their individual `accept()` and
`reject()` operations are not supported. This does not make collection-wide decisions partial:
`acceptAll()` and `rejectAll()` use one canonical transaction and refuse the entire sync if any
unsupported structural revision remains. No listed revision is resolved in that case. Handle this
`NotImplemented` refusal and leave the document unchanged, or let a reviewer resolve the structural
markup in Word. A `Body.revisions` collection in a header, footer, or note resolves only that story;
other stories sharing its OOXML part are left alone. Browser decisions share the editor's Undo stack;
one collection-wide decision is one Undo unit. Saving and reopening through the editor facade
preserves the resulting OOXML but intentionally starts a new browser session, so Undo and selection
do not survive the reopen.
