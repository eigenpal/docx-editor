...
For an exhaustive signature report, run this command from the repository root:

```bash
bun run --filter '@docx-editor.dev/editor-api' compat:report
```

The report compares the pinned Word and OfficeExtension inventory with actual
public source exports. It lists matching, differing, and missing signatures,
