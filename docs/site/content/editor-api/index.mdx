...
The `text` property and a search without options use the `all` projection. This compatibility
projection includes ordinary text, pending deletions, pending insertions, and both sides of a
pending replacement.
...
Use the same projection for the read and the search:

```ts
const body = context.document.body;
const text = body.getText({ projection: 'vanilla' });
...
A range returned by a `vanilla` search keeps that projection. Its `text` property, nested
searches, and `getText()` calls use the same view. The range endpoints remain model offsets, so
`insertText()`, `insertComment()`, and `select()` target the text that the search returned.
