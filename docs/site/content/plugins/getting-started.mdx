...
description: "Design reference for docx-editor plugins. EditorPlugin and CorePlugin registration are not shipped on the greenfield adapters yet."
category: "Plugins"
order: 71
seoTitle: 'Getting started with plugins'
---

> **Status:** design reference only. The greenfield React and Vue adapters do not ship `@docx-editor.dev/react/plugin-api`, `PluginHost`, or the `/ui` toolbar subpath described below. Revisit this page when plugin packaging lands in task 11.x.
...
```tsx
import type { PluginHostRef } from '@docx-editor.dev/react/plugin-api';
