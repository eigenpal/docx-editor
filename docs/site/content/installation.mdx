...
- Without the stylesheet the editor works but the chrome renders unstyled. The stylesheet ships precompiled and self-contained: every rule is scoped under `.docx-editor`, so it cannot leak into your app, cannot collide with your own Tailwind setup, and needs no Tailwind installed to work.
