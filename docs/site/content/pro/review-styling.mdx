...
The change bar is the hairline in the left margin beside every changed line. Two
tokens control it. The bar is painted on the page sheet, outside the dark-mode
inversion, so define both light and dark values when you change the color.
...
`footnote`, or `endnote`. While a header or footer is being edited, the bars of
the other stories dim with the text beside them.
