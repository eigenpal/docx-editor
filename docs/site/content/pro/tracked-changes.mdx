...
| `activeKey`                     | The key of the item that contains the caret, or `null`.                                                                                       |
| `setActive(key)`                | Select an item's range, scroll to it, and enter its story when needed. Returns `false` when the engine refuses activation. See `activatable`. |
