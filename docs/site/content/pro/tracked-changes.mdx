...
A changed paragraph mark keeps its pilcrow hidden during ordinary editing. If
you select its review card, every break in that decision shows its colored
pilcrow. The card shows the break count when one decision covers multiple
breaks. Margin change bars remain visible, including inside table cells.
