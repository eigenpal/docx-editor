...
Use `editor.setTrackedChangesFilter(predicate)` for filters that combine revision data.
The predicate receives a complete `ReviewRevisionItem`. Return `true` to keep its tracked
markup. Return `false` to render its accepted result.

The item includes `author`, `date`, `revisionKind`, `text`, `replacedText`, `ranges`,
`addresses`, `nesting`, `readOnly`, and pairing metadata. Dates can be absent or invalid.
Treat them as document data and validate them before comparison.

This filter shows insertions from Jess after February 1, 2026:
...
The editor evaluates the predicate once per revision item when the filter or document changes.
It then caches the result for pagination. Keep the predicate synchronous and free of side effects.
Call `setTrackedChangesFilter` again after captured filter state changes, even with the same
function reference. Pass `null` to clear the predicate.

The filtered view changes layout, painted markup, review cards, and bulk actions over the review
list. It does not apply review operations or change saved OOXML. Comments are unchanged unless
the separate reviewer filter hides their author. Both filters compose when they are active.
