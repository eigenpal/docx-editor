...
description: 'Suggesting mode records edits as Word revisions. Render them, accept and reject them, from the packaged sidebar or your own UI.'
category: 'Pro'
order: 61
---
...
`Delete` is the destructive one, and it is on both kinds of card: on a comment it deletes the thread, and on a tracked change it discards the suggestion. It shows only on the OPEN card — a rail where every card offers to throw a remark away invites clicking one by mistake — and it is absent on a card with nothing to discard. `Replies` draws one per reply as well, so a single answer can be taken back without deleting the conversation it belongs to.
...
| `gap`        | —       | Pixels between stacked cards. The only source of vertical spacing.                                     |
