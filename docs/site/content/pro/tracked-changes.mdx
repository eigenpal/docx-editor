...
`true` to keep the revision as tracked markup and in the review list. Return `false` to render
the accepted projection and remove its revision card.

| Revision kind                 | Result when the predicate returns `false`                     |
| ----------------------------- | ------------------------------------------------------------- |
| `insert`, `moveTo`            | The proposed content remains as ordinary content.             |
| `delete`, `moveFrom`          | The content is omitted from the filtered view.                |
| `replace`                     | The replacement remains and the replaced content is omitted.  |
| `format`                      | The current formatting remains without tracked-change markup. |
| `paragraphMark`, `structural` | The accepted paragraph or document structure is rendered.     |
...
review list. It does not accept or reject revisions, mutate the document, or change saved OOXML.
