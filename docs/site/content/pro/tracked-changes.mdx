...
In viewing mode, the packaged Accept, Reject, and Delete controls remain visible but disabled, with
`Read-only, no edits` as their title. The decision stays available to inspect and activate without
presenting a live control whose write the engine will refuse.
