...
express, per author. It is named for what it styles — the declaration renders no element of
its own, so the classes land on the author's painted spans, not on a wrapper. Keep those rules metric-safe. Outlines, shadows, and background accents are
...
- `data-review-author-slot` — their ramp slot (`0`–`7`), for styling by position rather than by
  name.
- `--doc-review-author-current` — a CSS variable holding that author's resolved color, which the
...
.docx-review__card[data-review-author='AI Assistant'] {
  background: #fef2f2;
  font-weight: 600;
}

/* Or by slot, when you do not know the names in advance. */
.docx-review__card[data-review-author-slot='2'] {
  outline: 1px solid var(--doc-review-author-current);
}
```
...
`data-review-author`, and it also has `data-review-author-slot` — the same number the
matching card carries — unless you mounted `ColorByChangeType` with no author
declarations. Use them for anything the declaration's typed fields do not cover:
...
.docx-editor [data-review-author-slot='1'] {
...
One pair of attributes covers everything with an author — tracked-change spans, comment
highlight bands, cards, hover balloons, and gutter markers all carry `data-review-author`
and `data-review-author-slot`, with the same slot number for the same person. So a rule for
one reviewer is one selector:
...
What differs is how you reach the color. Cards, balloons, markers, and bands also carry
`--doc-review-author-current`, holding that author's resolved color, so a rule on them can
