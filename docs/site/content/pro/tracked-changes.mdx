...
Two presses on the same run produce one card, because the record holds the
properties the run started with. Setting a property back to the value the record
holds removes the record.
...
Formatting text inside your own pending insertion records nothing either: the
whole run is already your proposal, so rejecting the insertion takes the words
and the formatting together. Formatting text inside another author's pending
insertion does record a change, and it is theirs to accept or reject.
