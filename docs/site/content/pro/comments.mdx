---
title: 'Comments'
description: 'Comment threads anchored to a range: read them, write them, reply, and render your own comment UI over the review hook.'
category: 'Pro'
order: 62
---
  ...
  return (
    <ul>
      {items.map((item) => (
        <li key={item.key}>
          <p>{item.text}</p>
          <span>{item.author}{item.date ? ` · ${item.date}` : ''}</span>
...
- [Tracked changes](/docs/2.x/pro/tracked-changes): the full `useReview` surface and the sidebar parts
