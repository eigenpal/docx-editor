...
Comments-only document protection permits these comment actions while keeping
document content locked. Read-only protection and `mode="view"` refuse comment
writes. See [Document protection](/docs/2.x/guides/document-protection) for the
rules for each restriction.
