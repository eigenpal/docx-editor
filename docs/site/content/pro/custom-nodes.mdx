...
`toDocx` is optional. Declare it and the payload is the only thing a write needs, because the tag and the text are computed from it. Without it, pass `attrs` and `text` on every call and keep them consistent yourself.
