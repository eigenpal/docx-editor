...
`schema` is any zod schema. The payload is validated against it before it is written and comes back as that type when read. `text` computes what the paragraph shows, so it stays in step with the data.
...
Two operations, separate on purpose.
...
`customNodeXml` writes nodes with no browser and `exportCustomNodes` removes them with no browser, so a document can be generated, stored with its payloads intact, and stripped on the way out without an editor being mounted. `packages/pro/src/__tests__/custom-node-server.test.ts` runs that path with the DOM globals deleted.
...
A bound control is read-only in Word — Word renders its text from the payload and does not accept typing into it — so the payload is the source of truth rather than a cache of the rendered text.
