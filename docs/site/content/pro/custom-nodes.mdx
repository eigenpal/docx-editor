...
Module registration is construction-time. Build the array outside render; a new array each render rebuilds the editor.
...
| `tagPrefix`                      | `string`                                                        | The prefix this definition claims. `docx` claims `docx:*`. No colons.                                     |
| `schema`                         | [Standard Schema](https://standardschema.dev)                   | Shape of the payload. zod, valibot and arktype all implement it. Nothing is bundled.                      |
| `toDocx`                         | `(data) => { attrs, text }`                                     | Derives the tag attrs and the document text from the payload.                                             |
| `fromDocx`                       | `({ attrs, text, data }) => attrs \| null`                      | Recognition hook. Return `null` to leave the control literal.                                             |
| `reviewCard`                     | `({ attrs, text, data }) => { title, detail } \| null`          | Contributes a sidebar card per node.                                                                      |
| `preserveOnExport`               | `boolean \| 'text'`                                             | What happens to the node on export. Defaults to `true`. See [Saving and exporting](#saving-and-exporting). |
| `payloadNamespace`               | `string`                                                        | Which customXml store the payloads live in. Defaults to one derived from `tagPrefix`.                     |
| `label`                          | `string`                                                        | Display name for chrome. Defaults to `name`. Localize it yourself.                                        |
| `chrome.color`                   | `string`                                                        | Chip tint and border, applied by `CustomNodeChrome`.                                                      |
| `onClick` / `onHover` / `onEdit` | `(node: ActivatedCustomNode) => void`                           | Interaction hooks on the painted chip.                                                                    |

### `toDocx` and `fromDocx` are not a pair

They read like a codec. They are not, and which one you need depends on where your node keeps its data.

`toDocx` runs on the way **out**: `data → { attrs, text }`. It exists so a write takes one argument instead of three you have to keep consistent. It needs a `schema`, because otherwise there is no typed payload to derive from.

`fromDocx` runs on the way **in**, and its job is recognition, not decoding. It receives the `w:tag` attrs — a string-to-string bag out of a file the sender controls — and returns the attrs the node should carry, or `null` to disown the control and leave it literal. That veto is the part nothing else does.

The asymmetry is the point: for a node with a payload, `toDocx`'s inverse is not a hook at all. The payload round-trips through the schema on its own, so `data` arrives typed with nothing declared.

| Your node keeps its data in | Declare                | Because                                                            |
| --------------------------- | ---------------------- | ------------------------------------------------------------------- |
| A payload                   | `schema` + `toDocx`    | The payload reads back typed by itself. Nothing to decode.          |
| The tag only                | `fromDocx`             | Tag attrs are untrusted strings. Clamp, normalize, or veto here.    |
| A payload, migrating from the tag | both              | `fromDocx` reads the old attrs; new writes go through `toDocx`.     |
...
| `attrs` | `Record<string, string>`                                       | Tag attrs. Derived by `toDocx` when declared. Passing it overrides the derivation. |
| `text`  | `string`                                                       | Document text. Derived by `toDocx` when declared.                                  |
...
`{ ok: true, changed: true }`, or a refusal:
...
| `notFound`    | No document is mounted, or no node has that id.                                                                       |
...
| `path`    | `(string \| number)[]` | Route to the field: `['authors', 0]`. |
...
`editor.save()` preserves everything. Use it for your own storage: the returned bytes reopen in this editor with their nodes intact.
...
| `'text'`           | The control is unwrapped. The text remains; the tag, binding and payload go. | A citation: the words are the point, the markup is yours. |
...
`preserveOnExport` is declared per definition, so one document can hold nodes with all three settings and each is decided independently. A definition you do not pass to `exportCustomNodes` is not touched at all — the function applies your policy to your own markup, it does not scrub a document.
...
The pairing is deliberate: `customNodeXml` writes the nodes with no browser, and `exportCustomNodes` removes them with no browser. A document can be generated, stored with its payloads intact, and stripped on the way out, without an editor ever being mounted. `packages/pro/src/__tests__/custom-node-server.test.ts` runs that whole path with the DOM globals deleted.

## Read

Inside `fromDocx`, `toDocx` and `reviewCard`, `data` is the schema's output type. Elsewhere it is `unknown`, because those surfaces carry every definition's nodes under one type.
...
// Every node in the document, with its range. Requires the review module.
...
| A chip cut to the clipboard       | The payload does not travel with it. Pasting produces a node with no data. |

The open-time sweep is not undoable. It collects payloads whose control was already gone when the document arrived, so undo immediately after opening a file must not restore them.

A bound control is read-only in Word: Word renders its text from the payload and does not accept typing into it. The payload is the source of truth, not a cache of the rendered text.
...
`w:storeItemID` matches `ds:itemID` in `customXml/itemProps1.xml`. That pair is the only link between the body and the payload.
...
Payloads are capped at 256 KB and labels at 4096 characters, enforced on write and on read. Keys named `__proto__`, `constructor` and `prototype` are stripped from a parsed payload at every depth.
...
A template engine can splice markup but cannot author a package, so `built.store` returns the parts you have to add: both `customXml` parts, both relationships, and the content-type override. All four are required — a control whose binding names a store that is not there is a document Word offers to repair.
...
A document opened without your definition registered renders the control's content literally, which is what Word does. Nothing is lost.
...
- Clipboard: cutting a chip carries the control, not the payload.
