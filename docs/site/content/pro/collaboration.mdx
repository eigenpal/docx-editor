...
Full-document rooms use shared schema version 3. All room participants must run the same editor release.
Version 3 preserves shared character identities across repeated formatting, concurrent typing, and undo.
Earlier releases cannot interpret these text ranges. New clients reject older persisted rooms with `schema-version-mismatch`.
To upgrade a persisted room, export its DOCX with the previous release. Upgrade every participant, then seed a new room from that DOCX.
This transfer preserves document content, but starts new collaboration history.
