...
`room.session.gateOperations(ops, scope)` before committing edits. Stop if it returns an error code.
A successful local transaction does not prove that peers received the edit.
If the session becomes incompatible, save the local document before replacing the store.
