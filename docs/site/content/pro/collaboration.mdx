...
Use the dedicated [collaboration format version](/docs/latest/pro/collaboration-versions)
to check clients before synchronization. Check saved rooms separately before
loading them. The guide covers server admission, offline work, and migration.
...
changes. Waiting does not fix the replica.

For `collaboration-format-mismatch`, `protocol-version-mismatch`, or
`schema-version-mismatch`, preserve local work
and follow [collaboration upgrades](/docs/latest/pro/collaboration-versions).
Rejoining the same incompatible room cannot resolve these errors.
...
The provider preserves recognized collaboration failure codes from the server
both during the initial join and after reconnecting. For
`collaboration-format-mismatch`, follow the
[upgrade guide](/docs/latest/pro/collaboration-versions); refreshing credentials
cannot resolve a version mismatch.

An unrecognized authentication refusal produces `initialization-aborted` during
initialization or `authentication-failed` after joining. For a rejected
credential, refresh it before calling `rejoin`. A `transport-disconnected`
...
Collaboration has a dedicated compatibility contract, independent of package
release numbers. Check clients before synchronization and check saved rooms
before loading them. The transport hooks do not configure your server's
admission policy.

See [Collaboration versions and upgrades](/docs/latest/pro/collaboration-versions)
for the client/server setup and deployment checks.

### Upgrade a persisted room

When a release requires a room upgrade, export with the compatible previous
build and create a fresh room from the verified DOCX. Keep local work and
backups. Collaboration undo history starts afresh.

Follow [Upgrade saved rooms](/docs/latest/pro/collaboration-versions#upgrade-saved-rooms)
for the complete procedure, including offline edits and rollback.

### Recover from an error

An `error` session cannot repair itself. First inspect the failure code.
For `collaboration-format-mismatch`, `schema-version-mismatch`, or
`protocol-version-mismatch`, stop reconnect
attempts: connect a compatible application version to that room, or follow the
[saved-room upgrade procedure](/docs/latest/pro/collaboration-versions#upgrade-saved-rooms). A fresh `Y.Doc` joining the same
...
session becomes incompatible, even though those edits cannot synchronize.
Preserve the local document for recovery. Treat the session's terminal error as
...
Repeated concurrent formatting, typing, deletion, and undo preserve document text
across participants.
