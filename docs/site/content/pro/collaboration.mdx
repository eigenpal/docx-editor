...
`error` is a `CollaborationFailure`. Branch on `error.code` instead of matching
message strings.
...
`useCollaborationStatus(session)` returns the live status and last failure.
`useCollaborationParticipants(session)` returns the room participants. Both
APIs are available from `@docx-editor.dev/pro/react` and
`@docx-editor.dev/pro/vue`.
...
After a successful join, rejection sets the status to `error` with
`authentication-failed`. `syncedTimeoutMs` limits the initial sync wait. Its
