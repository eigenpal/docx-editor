...
A background worker can join the same Hocuspocus room as the browser peers with `role: 'agent'`, then attach `DocxEditor.createCollaborative` as above. The worker owns its connection, so closing the initiating browser does not stop its job.
