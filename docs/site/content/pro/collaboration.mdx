...
Use the explicit proposal methods to create Word tracked changes:
...
await runtime.run(async (context) => {
  ...
  range.proposeReplacement('within 30 days');
  await context.sync();
});
```

`proposeInsertion(text, 'Before' | 'After')` and `proposeDeletion()` use the same
transaction path. Configure a non-empty runtime `author`; ordinary `insertText`
continues to make ordinary edits. Proposals support inline, single-paragraph
ranges, including table cells. Touching or overlapping pending revisions refuse;
resolve those decisions before proposing at their boundaries.
