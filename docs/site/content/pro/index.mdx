...
Capabilities are modules you pass to the editor root. Module registration happens at construction, like `mode`. The editor reads the array when it is created and when `document`, `fonts`, `translate`, or `imageDecodePort` identity changes. Build the array outside the component or memoize it so the first mount sees the modules you intend.
...
Everything the packaged sidebar renders is reachable from `useReview()`. Use the sidebar for Word-like cards out of the box, or the hook to render your own markup. React imports `@docx-editor.dev/pro/react`; Vue imports `@docx-editor.dev/pro/vue`.
