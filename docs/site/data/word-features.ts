...
export const wordFeatures: WordFeature[] = [
  ...
  },
  {
    ...
    tier: 'community',
    notes:
      "Each control accepts only the value its own type allows. A dropdown must name an item it declares, and a combo box also takes free text. A date validates an ISO instant and writes both `w:fullDate` and the formatted text. A checkbox writes its declared glyph and its state together. Editor checkbox toggles use MS Gothic when the state omits its font. The first write replaces a literal prompt whole, so clearing the value later leaves the control empty. A `w:temporary` control removes its own wrapper on the first edit and keeps the content. The value button sits past the control's right edge and scales with zoom; menus open under the control's left edge and stay on the page. The date picker is a month grid with a Today button that follows the editor locale for names and first weekday. Host renderers receive dropdown, combo box, date, and checkbox presses as sessions.",
    docsLink: '/docs/2.x/guides/content-controls',
  },
  {
