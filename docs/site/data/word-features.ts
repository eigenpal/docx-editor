...
export const wordFeatures: WordFeature[] = [
  ...
  },
  {
    ...
    tier: 'community',
    notes:
      'Register custom fonts with the fonts prop. The editor fetches your URLs and checks each hash. Theme fonts come from the OOXML theme. Word-accurate wrap and pagination need real font bytes, so the optional @docx-editor.dev/fonts package supplies substitutes for common Word fonts. Word\'s five document defaults match advance widths exactly. Century Gothic loads on demand and runs within 1%, so a wrap point can still move. Its googleFonts() resolver adds a pinned open-licensed catalog. A family with no metric-compatible substitute keeps your own measurement rather than being given an arbitrary face. packagedFonts() serves them on demand, loading only the families a document declares and reaching no third party; googleFonts() covers more families from a pinned catalog once an app opts into the network. Both are resolvers with the same call shape, so useFonts and useDocxSource compose them as a list in precedence order, and a later origin is told which FACES an earlier one can already paint, so composing them never downloads the same face twice.',
  },
  {
