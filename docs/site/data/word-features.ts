...
export const wordFeatures: WordFeature[] = [
  ...
  },
  {
    ...
    tier: 'community',
    notes:
      'Both adapters expose contextual toolbar controls that set borders and fill on the selected cells. Authored table and cell borders and table-style shading render and round-trip. A rule that two cells share paints once, centered on the boundary between them. With no compatibility mode, or mode 11, 12, or 14, simple single side rules are centered on the grid line. This applies to top-level, unpositioned, left-to-right tables without cell spacing. If the table has an absolute width, its side cell margins start at the center of the rule. In compatibility mode 15, both apply to centered tables with an absolute width. They also apply to left-aligned or right-aligned tables with an absolute width and one simple rule width on every cell side. Autofit and right-aligned tables use this geometry only when their outer borders fit within the available width. Those tables put the outer edge of the rule on the aligned edge, so the grid moves inward by half a rule. Other tables start the side margins at the inner edge of the rule.',
  },
  {
    id: 'tables.merge',
  ...
  },
  {
    ...
    tier: 'community',
    notes:
      'Both adapters have a typed note model, note layout (pageBottom, beneathText, sectEnd, docEnd), scoped note editing, insert, delete, convert, and chrome slots. A footnote stays whole with its reference: when it cannot fit below the referencing line, the line moves to the next page instead of the note splitting. A reference in a table row keeps its note with that row. The row moves with the note only when the note can place fewer than two lines below the row; otherwise the note continues on the next page. A row that every cell lets split below the reference line continues on the next page below that split; w:cantSplit and exact-height rows, w:keepLines cell paragraphs, and widow-controlled cell paragraphs in Word 2013 layout do not split there, unless the row is too tall to stay whole. Paragraphs ahead of a moved reference return to the earlier page when they fit, unless they keep with the reference paragraph; multi-column sections keep that room. A reference on the second line of a widow-controlled paragraph keeps the first two lines on the page only when the whole note or two of its lines fit below them; otherwise both lines move with the note. The note area can use the space below the text of the last body line with single or multiple line spacing, while a line whose note starts on the same page keeps its full height above the notes in every column. Outside tables, only a note taller than the page note column splits across pages, and a note paragraph that splits follows w:widowControl. The w:separator rule takes its thickness and its offset above the baseline from the strikeout metrics of the run font. Overflow sheets retain separate page rectangles for painting and hit testing. Editing inside a note matches the body: lists, tables, content controls, pictures, fonts, comments, bookmarks, and page setup. Suggesting mode tracks an inserted reference and requires reference deletion to propose note removal. Notes in headers and footers are out of scope.',
