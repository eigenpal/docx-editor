...
export const wordFeatures: WordFeature[] = [
  ...
  },
  {
    ...
    tier: 'community',
    notes:
      'FORMTEXT supports partial text edits, whole-field replacement in unprotected documents, and a shared React/Vue options dialog through double-click or the keyboard-accessible Edit field context action. The dialog supports regular text, number, and date types, maximum length, listed formats, and fill-in enabled. Selection outlines distinguish a whole field from a caret. In documents protected for forms, plain text results remain fillable and Tab selects the next enabled text field. FORMCHECKBOX renders its checked or default state from w:ffData, and an explicit w:size sets the glyph size. FORMDROPDOWN renders the cached result, or the selected list entry when the file caches none. Field markers, instructions, and w:ffData round-trip, and tracked edits survive. Form-field shading applies unless w:doNotShadeFormData is set. Protected filling enforces maximum length and applies supported value formats on exit. Computed input types and unlisted format pictures remain preserved without protected filling. Checkbox and dropdown interaction are not built. Results with nested fields, revisions, or non-text structure cannot use the default-text dialog or protected filling.',
  },
