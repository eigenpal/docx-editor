...
- **`collaboration`** — provider-neutral collaboration contracts: the document port,
  session surface, failure codes, and status tracker. DOM-free and imports no CRDT
  or transport; replication implementations live outside core.
...
**ProseMirror is contained to `binding`.** `contracts`, `store`, `collaboration`,
`layout` and `output` may not name a PM type or reach a PM view — enforced by identifier scan,
...
**Runtime-neutral lanes are DOM-free.** `store`, `collaboration`, `layout` and `automation` each
