...
Pass an i18n config to `<DocxEditor>`:

```tsx
import { DocxEditor } from '@eigenpal/docx-js-editor';
import de from '@eigenpal/docx-js-editor/i18n/de.json';

<DocxEditor i18n={{ locale: 'de', translations: de }} />;
```
...
### 5. Add your locale to the `Locale` type

In `src/i18n/LocaleContext.tsx`, expand the `Locale` union with your language tag:
...
// After (adding German)
export type Locale = 'en' | 'de' | (string & Record<never, never>);
```

This gives consumers autocomplete for your language in the `i18n` prop.
...
### Why `locale` matters for plurals
...
Without `locale`, the editor defaults to English rules — which means `few` and `many` branches in Polish/Arabic/Russian translations would never be selected. This is why `locale` exists in the `i18n` prop:

```tsx
import pl from '@eigenpal/docx-js-editor/i18n/pl.json';

// locale tells Intl.PluralRules which language's rules to apply
<DocxEditor i18n={{ translations: pl, locale: 'pl' }} />;
```

If your language only uses `one`/`other` (like English, German, French, Spanish), you can omit `locale` — it defaults to `'en'` which works fine.
