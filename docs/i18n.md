...
CLDR plural rules: `one` = 1, `few` = 2-4 (and 22-24, 32-34...), `many` = 5-21 (and 25-31...), `other` = fallback. Same pattern as `react-i18next` with `count` — just key-based lookup, no ICU parser needed.
