...
This example adds Italian. Choose a language that does not already exist. The command creates `packages/i18n/<lang>.json` with untranslated keys set to `null` and updates the typed exports in `packages/i18n/src/index.ts`. Edit the generated JSON. Use a BCP 47 language tag for `<lang>`.
