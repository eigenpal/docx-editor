...
The `locale` field in the `i18n` prop tells [`Intl.PluralRules`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl/PluralRules) which plural rules to use:

```tsx
import pl from '@eigenpal/docx-js-editor/i18n/pl.json';

<DocxEditor i18n={{ locale: 'pl', translations: pl }} />;
```
