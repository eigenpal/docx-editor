...
Use `tPlural()` for count-dependent strings. It supports CLDR plural categories (`one`, `few`, `many`, `other`) for Slavic languages, Arabic, and more.
...
tPlural('comments.replies', 5); // → "5 replies"
```

For Polish (with `lang="pl"` on DocxEditor), `few` and `many` resolve different word forms:
...
// 1 plik, 2 pliki, 5 plików, 22 pliki, 100 plików
```

Plural rules are handled by the browser's built-in [`Intl.PluralRules`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl/PluralRules) — the same standard that `react-i18next` and `next-intl` use. All languages are supported correctly out of the box.

Pass `lang` to enable correct plural rules:

```tsx
<DocxEditor locale={pl} lang="pl" />
```

## Limitations

- **No RTL layout** — String translation works, but the editor UI layout is not RTL-ready. RTL support is a separate effort.
- **No ICU MessageFormat** — No support for gender or ordinals. Simple `{variable}` interpolation and CLDR plurals only.
