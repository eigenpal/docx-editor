...
Each locale file includes a `_lang` key (e.g., `"_lang": "de"`) that tells [`Intl.PluralRules`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl/PluralRules) which plural rules to use. This is set automatically by `bun run i18n:new`. Just pass the locale — one prop:
