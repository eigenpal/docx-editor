...
Plurals use ICU MessageFormat syntax — same as `next-intl`. One key, inline rules, same key across all languages:
...
```tsx
const { t } = useTranslation();
t('comments.replyCount', { count: 5 }); // → "5 replies"
t('comments.replyCount', { count: 1 }); // → "1 reply"
```

Translators add their language's plural forms in the same key — no structural changes needed:
...
Pass `lang` to enable correct plural rules via [`Intl.PluralRules`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl/PluralRules):
