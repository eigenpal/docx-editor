...
The eight published packages and the private Nuxt package are in a fixed group in `.changeset/config.json`. Changesets assigns them the same release version.

Example applications and the Nuxt package are private workspaces (`"private": true`). Changesets can update their versions and changelogs, but they are not published to npm or tagged. The Release workflow omits private packages from the release PR description. Their version changes remain visible in the PR diff. Keep example applications private when adding them to the workspace.
...
| `.changeset/config.json` | `"access": "public"`; fixed release group for the eight published packages and private Nuxt; private versioning enabled and tagging disabled                                       |
