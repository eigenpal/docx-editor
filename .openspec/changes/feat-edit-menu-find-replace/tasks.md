...
**File:** `packages/react/src/components/paged-editor/OffscreenEditorHost.tsx` 4. Accept `spellCheckEnabled` prop 5. Set `spellcheck` attribute on ProseMirror EditorView via `attributes` config option 6. Update attribute when prop changes (via `EditorView.setProps` or `useEffect`)
