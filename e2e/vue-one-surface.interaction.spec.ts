// Vue one-surface interaction gate (interactive-paginated-editing M5.2).
//
// The Vue mirror of `react-one-surface.interaction.spec.ts`: the SAME eleven
// scenarios, driven the same way, against the Vue adapter. M5 passes only when
// Vue proves equivalent behaviour, so the two specs are kept deliberately
// parallel — a scenario that exists in one and not the other is a parity hole.
//
...
test.describe('Vue one-surface interaction (task M5.2)', () => {
