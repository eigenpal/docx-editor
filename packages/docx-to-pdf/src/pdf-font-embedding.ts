...
import type { ExportAdmittedFontFace as PdfAdmittedFont } from '@docx-editor.dev/core/export';
...
function sfntEmbeddingPermission(bytes: Uint8Array, base: number): FontEmbeddingDecision {
  ...
  for (let index = 0; index < tableCount; index += 1) {
    ...
    if ((fsType & 0x0100) !== 0) {
      return refuse(
        'The OS/2 fsType forbids subsetting, and this writer has no safe full-font embedding mode'
      );
    }
...
/** Decides whether this writer may embed an admitted face, including TTC/OTC selection. @internal */
