'use strict';
var chunkZ2UPNKQW_js = require('./chunk-Z2UPNKQW.js'),
  chunkUSRMBYI6_js = require('./chunk-USRMBYI6.js'),
...
function L(e) {
  return e != null && e !== '' && e !== 0 ? String(e) : String(j++);
}
var fe = new Set([
  'black',
  'blue',
...
function b(e) {
  ...
  if (
    ...
    e.highlight && e.highlight !== 'none')
  ) {
    if (fe.has(e.highlight)) t.push(`<w:highlight w:val="${e.highlight}"/>`);
    else if (!e.shading) {
...
function de(e) {
  let t = Number.isInteger(e.info.id) && e.info.id >= 0 ? e.info.id : 0,
    ...
    c = [`w:id="${t}"`, `w:author="${s(r)}"`];
  (i && c.push(`w:date="${s(i)}"`), o && c.push(`w:rsid="${s(o)}"`));
  let a = b(e.previousFormatting) || '<w:rPr/>';
  return `<w:rPrChange ${c.join(' ')}>${a}</w:rPrChange>`;
}
function we(e, t) {
  let n = b(e),
    r = n ? he(n) : '',
...
function Fe(e) {
  ...
  if (e.type === 'solid' && e.color) return `<a:solidFill>${D(e.color)}</a:solidFill>`;
  if (e.type === 'gradient' && e.gradient) {
    let t = e.gradient,
      n = t.stops.map((i) => `<a:gs pos="${i.position}">${D(i.color)}</a:gs>`).join(''),
...
function O(e) {
  ...
  let n = [];
  return (
    e.color && n.push(`<a:solidFill>${D(e.color)}</a:solidFill>`),
...
function Se(e, t) {
  let n = e.size.width,
    ...
    i = e.rId || 'rId1',
    o = t,
    c = e.filename || `image${o}`,
    a = '';
  return (
    e.transform?.rotation && (a += ` rot="${Math.round(e.transform.rotation * 6e4)}"`),
    e.transform?.flipH && (a += ' flipH="1"'),
    e.transform?.flipV && (a += ' flipV="1"'),
    [
      '<a:graphic xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main">',
      '<a:graphicData uri="http://schemas.openxmlformats.org/drawingml/2006/picture">',
      '<pic:pic xmlns:pic="http://schemas.openxmlformats.org/drawingml/2006/picture">',
      '<pic:nvPicPr>',
      `<pic:cNvPr id="${o}" name="${s(c)}"${e.alt ? ` descr="${s(e.alt)}"` : ''}/>`,
      '<pic:cNvPicPr/>',
      '</pic:nvPicPr>',
      '<pic:blipFill>',
      `<a:blip r:embed="${i}"/>`,
...
function Re(e) {
  let t = e.image,
    ...
    c = t.padding?.bottom ?? t.wrap.distB ?? 0,
    a = t.padding?.left ?? t.wrap.distL ?? 0,
    l = t.padding?.right ?? t.wrap.distR ?? 0,
    p = L(t.id),
  ...
  if (!n)
    return [
      '<w:drawing>',
      `<wp:inline distT="${o}" distB="${c}" distL="${a}" distR="${l}">`,
      `<wp:extent cx="${r}" cy="${i}"/>`,
      '<wp:effectExtent l="0" t="0" r="0" b="0"/>',
      `<wp:docPr id="${p}" name="${s(f)}"${t.alt ? ` descr="${s(t.alt)}"` : ''}${t.decorative ? ' hidden="1"' : ''}/>`,
  ...
  return [
    '<w:drawing>',
    `<wp:anchor distT="${o}" distB="${c}" distL="${a}" distR="${l}" simplePos="0" relativeHeight="251658240" behindDoc="${u}" locked="0" layoutInCell="1" allowOverlap="1">`,
    '<wp:simplePos x="0" y="0"/>',
    h,
    `<wp:extent cx="${r}" cy="${i}"/>`,
    '<wp:effectExtent l="0" t="0" r="0" b="0"/>',
    g,
    `<wp:docPr id="${p}" name="${s(f)}"${t.alt ? ` descr="${s(t.alt)}"` : ''}/>`,
    '<wp:cNvGraphicFramePr><a:graphicFrameLocks xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main" noChangeAspect="1"/></wp:cNvGraphicFramePr>',
    m,
    '</wp:anchor>',
    '</w:drawing>',
...
function ze(e) {
  ...
  if (!o)
    return [
      '<w:drawing>',
      `<wp:inline distT="${c}" distB="${a}" distL="${l}" distR="${p}">`,
      `<wp:extent cx="${n}" cy="${r}"/>`,
      '<wp:effectExtent l="0" t="0" r="0" b="0"/>',
      `<wp:docPr id="${f}" name="${s(m)}"/>`,
      '<wp:cNvGraphicFramePr/>',
      E,
      '</wp:inline>',
      '</w:drawing>',
    ].join('');
  let se = t.wrap?.type === 'behind' ? '1' : '0',
    ae = t.position
      ? M(t.position)
      : '<wp:positionH relativeFrom="column"><wp:posOffset>0</wp:posOffset></wp:positionH><wp:positionV relativeFrom="paragraph"><wp:posOffset>0</wp:posOffset></wp:positionV>',
    ce = H(t.wrap);
  return [
    '<w:drawing>',
    `<wp:anchor distT="${c}" distB="${a}" distL="${l}" distR="${p}" simplePos="0" relativeHeight="251658240" behindDoc="${se}" locked="0" layoutInCell="1" allowOverlap="1">`,
    '<wp:simplePos x="0" y="0"/>',
    ae,
    `<wp:extent cx="${n}" cy="${r}"/>`,
    '<wp:effectExtent l="0" t="0" r="0" b="0"/>',
...
function y(e) {
  let t = [],
    n = we(e.formatting, e.propertyChanges);
  n && t.push(n);
...
function W(e, t) {
  ...
  return (
    t && t.length > 0 && n.push(...t.map((r) => Me(r))),
...
function Me(e) {
  let t = Number.isInteger(e.info.id) && e.info.id >= 0 ? e.info.id : 0,
    ...
    c = [`w:id="${t}"`, `w:author="${s(r)}"`];
  (i && c.push(`w:date="${s(i)}"`), o && c.push(`w:rsid="${s(o)}"`));
  let a = W(e.previousFormatting) || '<w:pPr/>',
    l = Oe(a),
    p = l.length > 0 ? `<w:pPr>${l}</w:pPr>` : '<w:pPr/>';
  return `<w:pPrChange ${c.join(' ')}>${p}</w:pPrChange>`;
}
function k(e) {
  let t = [];
...
function He(e) {
  ...
  let c =
    e.instruction.startsWith(' ') || e.instruction.endsWith(' ') || e.instruction.includes('  ')
      ? ' xml:space="preserve"'
      : '';
  (t.push(`<w:r>${r}<w:instrText${c}>${s(e.instruction)}</w:instrText></w:r>`),
    t.push(`<w:r>${r}<w:fldChar w:fldCharType="separate"/></w:r>`));
  for (let a of e.content) a.type === 'run' && t.push(y(a));
...
function z(e, t) {
  let n = t.info,
    ...
    c = typeof n.date == 'string' ? n.date.trim() : void 0,
    a = [`w:id="${r}"`, `w:author="${s(o)}"`];
  c && a.push(`w:date="${s(c)}"`);
  let l = t.content
    .map((p) =>
      p.type === 'run'
...
function Ge(e) {
  switch (e.type) {
    ...
    case 'moveFromRangeStart':
      return N('moveFromRangeStart', e);
    case 'moveFromRangeEnd':
      return `<w:moveFromRangeEnd w:id="${e.id}"/>`;
    case 'moveToRangeStart':
      return N('moveToRangeStart', e);
    case 'moveToRangeEnd':
...
function $(e) {
  ...
  let r = n.length > 0 ? ' ' + n.join(' ') : '',
    i = W(e.formatting, e.propertyChanges);
  i && t.push(i);
...
function q(e, t) {
  ...
  if (e) {
    ...
    o && n.push(o);
    let c = T(e.indent, 'tblInd');
    c && n.push(c);
    let a = V(e.borders, 'tblBorders');
    a && n.push(a);
    let l = U(e.cellMargins, 'tblCellMar');
    (l && n.push(l), e.layout && n.push(`<w:tblLayout w:type="${e.layout}"/>`));
    let p = Z(e.shading);
    p && n.push(p);
    let f = Ve(e.look);
  ...
  return (
    t && t.length > 0 && n.push(...t.map((r) => qe(r))),
...
function qe(e) {
  let t = P(e.info, e.info.rsid),
    n = q(e.previousFormatting) || '<w:tblPr/>',
    r = Ze(n),
...
function Y(e, t, n) {
  ...
  return (
    ...
    t && t.length > 0 && r.push(...t.map((i) => Je(i))),
...
function J(e, t, n) {
  ...
  if (e) {
    ...
    let c = V(e.borders, 'tcBorders');
    c && r.push(c);
    let a = Z(e.shading);
    (a && r.push(a), e.noWrap && r.push('<w:noWrap/>'));
    let l = U(e.margins, 'tcMar');
    (l && r.push(l),
  ...
  return (
    ...
    t && t.length > 0 && r.push(...t.map((i) => et(i))),
...
function et(e) {
  let t = P(e.info, e.info.rsid),
    n = J(e.previousFormatting) || '<w:tcPr/>',
    r = Qe(n),
...
function nt(e) {
  let t = [];
  for (let n of e) n.type === 'paragraph' ? t.push($(n)) : n.type === 'table' && t.push(R(n));
  return (t.length === 0 && t.push('<w:p/>'), t.join(''));
}
function rt(e) {
  let t = [],
    n = J(e.formatting, e.propertyChanges, e.structuralChange);
  return (n && t.push(n), t.push(nt(e.content)), `<w:tc>${t.join('')}</w:tc>`);
}
function it(e) {
  let t = [],
    n = Y(e.formatting, e.propertyChanges, e.structuralChange);
  n && t.push(n);
  for (let r of e.cells) t.push(rt(r));
  return `<w:tr>${t.join('')}</w:tr>`;
}
function R(e) {
  let t = [],
    n = q(e.formatting, e.propertyChanges);
  n && t.push(n);
  let r = tt(e.columnWidths);
  r && t.push(r);
  for (let i of e.rows) t.push(it(i));
...
function wt(e) {
  if (!e) return '';
  let t = [];
  if (e.headerReferences) for (let f of e.headerReferences) t.push(st(f));
  if (e.footerReferences) for (let f of e.footerReferences) t.push(at(f));
  let n = ct(e.footnotePr);
  n && t.push(n);
  let r = lt(e.endnotePr);
...
function K(e) {
  ...
  if (e.type === 'blockSdt') {
    let t = e.content.map((i) => K(i)).join(''),
      n = e.properties,
      r = [];
    return (
...
function gt(e) {
  return e.map((t) => K(t)).join('');
}
function yt(e) {
  let t = [];
  return (
    t.push(gt(e.content)),
    e.finalSectionProperties && t.push(wt(e.finalSectionProperties)),
    t.join('')
  );
}
function Q(e) {
  ...
  return (
    ...
    t.push('<w:body>'),
    t.push(yt(e.package.document)),
    t.push('</w:body>'),
    t.push('</w:document>'),
    t.join('')
  );
}
var A = chunkH5NTJZO4_js.c(chunkUSRMBYI6_js.a());
var xt = {
  wpc: 'http://schemas.microsoft.com/office/word/2010/wordprocessingCanvas',
  mc: 'http://schemas.openxmlformats.org/markup-compatibility/2006',
  o: 'urn:schemas-microsoft-com:office:office',
...
function $t() {
  return Object.entries(xt)
    .map(([e, t]) => `xmlns:${e}="${t}"`)
    .join(' ');
}
function Tt(e) {
  return e.type === 'paragraph' ? $(e) : e.type === 'table' ? R(e) : '';
}
function ee(e) {
  let t = e.type === 'header' ? 'w:hdr' : 'w:ftr',
    n = $t(),
    r = e.content.map((i) => Tt(i)).join('');
  return (
    r || (r = '<w:p><w:pPr/></w:p>'),
...
function Pt(e) {
  let t = '<w:p>';
  for (let n of e.content) n.type === 'run' && (t += ne(n));
  return ((t += '</w:p>'), t);
}
function bt(e) {
  let t = '<w:p>';
...
function te(e) {
  ...
  if (e.content && e.content.length > 0) {
    n += bt(e.content[0]);
    for (let r = 1; r < e.content.length; r++) n += Pt(e.content[r]);
  } else
    n +=
...
<w:comments xmlns:wpc="http://schemas.microsoft.com/office/word/2010/wordprocessingCanvas" xmlns:mc="http://schemas.openxmlformats.org/markup-compatibility/2006" xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships" xmlns:m="http://schemas.openxmlformats.org/officeDocument/2006/math" xmlns:v="urn:schemas-microsoft-com:vml" xmlns:wp="http://schemas.openxmlformats.org/drawingml/2006/wordprocessingDrawing" xmlns:w10="urn:schemas-microsoft-com:office:word" xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main" xmlns:w14="http://schemas.microsoft.com/office/word/2010/wordml" xmlns:wpg="http://schemas.microsoft.com/office/word/2010/wordprocessingGroup" xmlns:wpi="http://schemas.microsoft.com/office/word/2010/wordprocessingInk" xmlns:wne="http://schemas.microsoft.com/office/word/2006/wordml" xmlns:wps="http://schemas.microsoft.com/office/word/2010/wordprocessingShape" mc:Ignorable="w14 wp14">`;
  for (let i of t) r += te(i);
  for (let i of n) r += te(i);
...
async function vt(e, t, n) {
  ...
  (t.file('word/comments.xml', i, { compression: 'DEFLATE', compressionOptions: { level: n } }),
    await Promise.all([It(t, n), Dt(t, n)]));
}
function ie(e) {
  let t = [];
  for (let n of e)
    if (n.type === 'paragraph') {
      ...
      for (let r of n.rows) for (let i of r.cells) t.push(...ie(i.content));
  return t;
}
var Ct = {
  'image/png': 'png',
...
function Ft(e) {
  ...
  return { data: r.buffer, extension: Ct[t[1]] || 'png' };
}
async function St(e, t, n) {
  ...
  let l = [],
    p = new Set();
  for (let f of e) {
    let { data: m, extension: u } = Ft(f.src);
    (a++, c++);
    let h = `image${a}.${u}`,
      g = `word/media/${h}`,
      F = `rId${c}`;
  ...
  if (
    (l.length > 0 &&
      ((o = o.replace('</Relationships>', l.join('') + '</Relationships>')),
...
function oe(e) {
  ...
  for (let n of e)
    if (n.type === 'paragraph')
      ...
      for (let r of n.rows) for (let i of r.cells) t.push(...oe(i.content));
  return t;
}
async function Rt(e, t, n) {
  ...
  for (let l of e) {
    ...
    let p = `rId${c}`;
    (a.push(
      `<Relationship Id="${p}" Type="${chunkZ2UPNKQW_js.a.hyperlink}" Target="${s(l.href)}" TargetMode="External"/>`
    ),
      (l.rId = p));
  }
  a.length > 0 &&
    ((o = o.replace('</Relationships>', a.join('') + '</Relationships>')),
    t.file(r, o, { compression: 'DEFLATE', compressionOptions: { level: n } }));
}
async function At(e, t = {}) {
  ...
  let { compressionLevel: n = 6, updateModifiedDate: r = true, modifiedBy: i } = t,
    o = e,
    c = await A.default.loadAsync(e.originalBuffer),
    a = new A.default();
  for (let [u, h] of Object.entries(c.files)) {
    if (h.dir) {
      a.folder(u.replace(/\/$/, ''));
      continue;
    }
    let g = await h.async('arraybuffer');
    a.file(u, g, { compression: 'DEFLATE', compressionOptions: { level: n } });
  }
  let l = ie(o.package.document.content);
  await St(l, a, n);
  let p = oe(o.package.document.content);
  await Rt(p, a, n);
  let f = Q(o);
  if (
    ...
    if (h) {
      let g = await h.async('text'),
        F = jt(g, { updateModifiedDate: r, modifiedBy: i });
      a.file(u, F, { compression: 'DEFLATE', compressionOptions: { level: n } });
    }
  }
...
var zt = 'application/vnd.openxmlformats-officedocument.wordprocessingml.comments+xml';
async function It(e, t) {
  ...
  r.includes('/word/comments.xml') ||
    ((r = r.replace(
      '</Types>',
      `<Override PartName="/word/comments.xml" ContentType="${zt}"/></Types>`
    )),
...
async function Dt(e, t) {
  ...
  ((i = i.replace(
    '</Relationships>',
    `<Relationship Id="${o}" Type="${chunkZ2UPNKQW_js.a.comments}" Target="comments.xml"/></Relationships>`
  )),
    e.file(n, i, { compression: 'DEFLATE', compressionOptions: { level: t } }));
}
async function pn(e, t, n = {}) {
  let r = await A.default.loadAsync(e);
  return kt(r, t, n);
}
async function kt(e, t, n = {}) {
  let { compressionLevel: r = 6 } = n;
  for (let [i, o] of t) e.file(i, o, { compression: 'DEFLATE', compressionOptions: { level: r } });
  return e.generateAsync({
    type: 'arraybuffer',
    compression: 'DEFLATE',
    compressionOptions: { level: r },
  });
}
function Bt(e) {
  let t = new Map(),
    n = e.package.relationships;
  if (!n) return t;
  let r = [
    { map: e.package.headers, type: chunkZ2UPNKQW_js.a.header },
    { map: e.package.footers, type: chunkZ2UPNKQW_js.a.footer },
  ];
  for (let { map: i, type: o } of r)
    if (i)
      for (let [c, a] of i.entries()) {
        let l = n.get(c);
        if (l && l.type === o && l.target) {
          let p = l.target.startsWith('/') ? l.target.slice(1) : `word/${l.target}`;
          t.set(p, ee(a));
        }
      }
  return t;
...
function Et(e, t, n) {
  let r = { level: n };
  for (let [i, o] of Bt(e)) t.file(i, o, { compression: 'DEFLATE', compressionOptions: r });
}
function jt(e, t) {
  let n = e;
