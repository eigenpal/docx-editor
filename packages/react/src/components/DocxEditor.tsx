...
interface EditorState {
  ...
  paragraphTabs: import('@eigenpal/docx-core/types/document').TabMark[] | null;
      ...
      if (view && selectionState.hasSelection && !isAddingComment && !readOnly) {
        ...
        if (container && parentEl) {
          ...
          if (pagesEl) {
            ...
            const spans = pagesEl.querySelectorAll('span[data-doc-from]');
            for (const span of spans) {
              const el = span as HTMLElement;
              const docFrom = Number(el.dataset.docFrom);
              const docTo = Number(el.dataset.docTo);
              if (selFrom >= docFrom && selFrom <= docTo) {
