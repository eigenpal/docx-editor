...
export function DocxEditorDialogs({
  ...
  // `pmTableContext` is gone: it carried a ProseMirror node's attrs, which this adapter
  // must not name, and the engine's table capability does not report those fields.
  editor: Editor | null;
  // Split cell
    ...
    <Suspense fallback={null}>
      ...
      {tablePropsOpen && (
        <TablePropertiesDialog
          ...
          onApply={(props) => {
            // Legacy dispatched a ProseMirror command here. Same intent through the
            // contract: the engine refuses `setTableProperties` today, so the dialog
            // applies nothing rather than appearing to work.
            editor?.exec({ type: 'setTableProperties', ...props });
          }}
          // `getSelectedTable` reports the table's shape (row/column counts), not its
          // preferred width or justification, so there is nothing to seed the fields
          // with — they open at their defaults rather than at invented values.
