 ...
 *
 * `editor` drives `canUndo` / `canRedo`, asked of the engine here rather
 * than in the orchestrator so the deps stay local. Legacy read the editing
 * engine's history depth directly; `can()` is the same question put to the
 * contract, and it answers by dry-running the command.
 */
  ...
  return (
    <div ref={toolbarRefCallback} className="z-50 flex flex-col gap-0 flex-shrink-0">
      <EditorToolbar
        ...
        onUndo={onUndo}
        onRedo={onRedo}
        canUndo={editor?.can({ type: 'undo' }).ok ?? false}
        canRedo={editor?.can({ type: 'redo' }).ok ?? false}
        disabled={readOnly}
