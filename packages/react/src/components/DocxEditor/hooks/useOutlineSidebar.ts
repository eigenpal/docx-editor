...
export function useOutlineSidebar({
  ...
  useEffect(() => {
    ...
    if (showOutlineProp) {
      // Legacy walked the ProseMirror doc; the engine derives the outline from the
      // authored model. `pmPos` keeps the legacy field name the panel reads — the
      // engine addresses blocks by id, so the ordinal stands in until the panel
      // scrolls by block id.
      const headings = editorRef.current?.getOutline() ?? [];
      setHeadingInfos(headings.map((h, i) => ({ text: h.text, level: h.level, pmPos: i })));
    }
  }, [showOutlineProp, editorRef]);
