...
import { bodyStoryId, isRunPropertiesCapsule, type PackageModel, type RunRecord } from '@docx-editor.dev/engine-core';
...
export function paragraphNodeToRuns(node: PMNode): RunRecord[] {
  ...
  node.forEach((child) => {
    ...
    for (const m of child.marks) {
      ...
      if (m.type.name === 'rawRunProps' && isRunPropertiesCapsule(String(m.attrs.rpr))) rPrCapsule = String(m.attrs.rpr);
    ...
    else runs.push(Object.keys(props).length > 0 ? { text: child.text, props } : { text: child.text });
  });
  return runs;
}
