...
for (const explicitTranslate of [false, true]) {
  for (const composed of [false, true]) {
    test(`${composed ? 'Root provider' : 'packaged editor'} keeps ${explicitTranslate ? 'explicit translate' : 'catalogue'} form labels live without replacing controls`, async () => {
      ...
      const fallbackCatalogue = { textFormField: { maxLength: 'Catalogue length' } };
      const translateProps = () => {
        if (!explicitTranslate) return {};
        const translate = createT(
          deepMerge(
        ...
        return {
          [composed ? 'translate' : 't']: (
            key: string,
            params?: Record<string, string | number>
          ) =>
            key === 'textFormField.maxLength'
              ? key
      ...
      const app = createApp({
        render: () =>
          composed
            ? h(
                LocaleProvider,
                { i18n: explicitTranslate ? fallbackCatalogue : catalogue.value },
                {
                  default: () =>
                    h(
                      DocxEditorRoot,
      ...
      try {
        ...
        expect(panel.textContent).toContain(
          explicitTranslate ? 'Catalogue length' : 'Maximum length'
        );
        ...
        expect(container.querySelector('.docx-text-form-status')?.textContent).toBe(
          'Field selected'
        );
      } finally {
        app.unmount();
