...
function awarenessPayload(value: unknown): AwarenessPayload | null {
  ...
  if (selection === undefined) {
    return { actorId: record.actorId, name: record.name, ...(color ? { color } : {}), role };
  }
...
class Session implements YjsCollaborationSession {
  ...
  setLocalSelection(selection: CollaborationLocalSelection | null): void {
    ...
    const text = selection ? paragraphs.get(selection.paragraphId.toUpperCase()) : undefined;
    const resolved =
      selection && text
        ? {
            paragraphId: selection.paragraphId.toUpperCase(),
            anchor: encodeBytes(
            ...
            head: encodeBytes(
              Y.encodeRelativePosition(
                Y.createRelativePositionFromTypeIndex(
                  text,
                  Math.max(0, Math.min(selection.end, text.length))
                )
              )
            ),
          }
  ...
  remoteSelections(): readonly CollaborationRemoteSelection[] {
    ...
    for (const [clientId, state] of states) {
      ...
      const text = paragraphs.get(payload.selection.paragraphId);
      if (!text) continue;
      let anchor: Y.AbsolutePosition | null;
      let head: Y.AbsolutePosition | null;
      try {
        anchor = Y.createAbsolutePositionFromRelativePosition(
          Y.decodeRelativePosition(decodeBytes(payload.selection.anchor)),
          this.ydoc
        );
        head = Y.createAbsolutePositionFromRelativePosition(
          Y.decodeRelativePosition(decodeBytes(payload.selection.head)),
          this.ydoc
        );
      } catch {
        continue;
      ...
      const paragraph = this.port
        .paragraphs()
        .find((candidate) => candidate.paragraphId === payload.selection!.paragraphId);
