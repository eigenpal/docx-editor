...
function awarenessPayload(value: unknown): AwarenessPayload | null {
  ...
  return {
    ...
    selection: {
      ...
      start: selected.start as number,
      end: selected.end as number,
    },
  };
}
...
class DocumentSession implements YjsCollaborationSession {
  ...
  setLocalSelection(selection: CollaborationLocalSelection | null): void {
    ...
    const paragraphId = selection.paragraphId.toUpperCase();
    this.publishAwareness({
      paragraphId,
  ...
  remoteSelections(): readonly CollaborationRemoteSelection[] {
    ...
    for (const [clientId, state] of states) {
      ...
      const paragraph = paragraphs.get(payload.selection.paragraphId);
      if (!paragraph) continue;
      const limit = paragraph.text.length;
      selections.push(
        Object.freeze({
          actorId: payload.actorId,
          name: payload.name,
          ...
          start: Math.min(payload.selection.start, limit),
          end: Math.min(payload.selection.end, limit),
        })
      );
    }
