...
describe('interaction planner (task 5.1)', () => {
  ...
  test('plans geometry keyboard through shared preconditions when frame is focused', () => {
    ...
    const unfocused = planInteraction(
      { ...plannerContext(frame), navigationGeometry: navigation },
      {
        kind: 'geometryKeyboard',
        frameId: frame.id,
        key: 'ArrowRight',
  ...
  test('plans blur, command, and native-input delegation engine effects', () => {
    const frame = publishFrame();
    expect(
      planInteraction(plannerContext(frame), { kind: 'blur', frameId: frame.id }).effects
    ).toEqual([{ kind: 'blur' }]);
    expect(
      planInteraction(plannerContext(frame), {
        kind: 'command',
    ...
    expect(
      planInteraction(plannerContext(frame), { kind: 'delegateNativeInput', frameId: frame.id })
        .effects
...
describe('body-paragraph interaction subset (task 5.6a)', () => {
  ...
  test('history and selection commands stay available over a read-only selection (5.6a)', () => {
    ...
    for (const command of [{ type: 'undo' as const }, { type: 'redo' as const }]) {
      expect(
        planInteraction(plannerContext(frame), { kind: 'command', frameId: frame.id, command })
          .effects
...
describe('synchronous stale-frame protection (task 5.7a)', () => {
  ...
  test('grapheme offsets beyond current canonical state are refused, not clamped', () => {
    ...
    const plan = planSelection(
      frame,
      selectionForBlock(frame, blockId, graphemeCount + 1, graphemeCount + 4)
    );
    expect(plan.effects).toHaveLength(1);
    expect(plan.effects[0]).toMatchObject({ kind: 'reject', code: 'invalidTarget' });
    expect(plan.effects.some((e) => e.kind === 'syncSelection')).toBe(false);
  ...
  test('an offset exactly at the trailing caret stop stays valid', () => {
    ...
    const plan = planSelection(
      frame,
      selectionForBlock(frame, block.identity.blockId, 0, block.graphemeCount)
    );
    expect(plan.effects.map((e) => e.kind)).toEqual(['syncSelection', 'focus']);
  });
  ...
  test('a block deleted from canonical state cannot receive a selection', () => {
    const frame = publishFrame();
    const source = selectionForBlock(
      frame,
      frame.semanticIndex.stories[0]!.blocks[0]!.identity.blockId,
      0,
      1
  ...
  test('resolution reads the frame handed to the planner, not a retained one', () => {
    ...
    expect(planSelection(first, wasValid).effects.map((e) => e.kind)).toEqual([
      'syncSelection',
      'focus',
    ]);
...
describe('setSelection refuses rather than clamps (independent review, HIGH)', () => {
  test('an out-of-range grapheme offset is refused, not silently relocated', () => {
    const frame = publishFrame();
    const block = frame.semanticIndex.stories[0]!.blocks[0]!;
    const beyond = selectionForBlock(
      frame,
      block.identity.blockId,
      block.graphemeCount + 500,
      block.graphemeCount + 900
    );
  ...
  test('a non-integer offset is refused rather than thrown', () => {
    ...
    expect(
      resolveSelectionAgainstCanonicalState(frame, selectionForBlock(frame, blockId, 1.5, 2))
    ).toMatchObject({
      kind: 'reject',
      code: 'invalidTarget',
    });
