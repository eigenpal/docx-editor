 ...
 * support. Deliberately small and auditable: it only ever runs, in the
 * scheduled drift-check job, against bytes whose sha512 already matched the
 * npm registry's published `dist.integrity` before this module sees them.
 */
...
export function parseTarEntries(tarBuffer) {
  ...
  while (offset + 512 <= tarBuffer.length) {
    ...
    if (REGULAR_FILE_TYPEFLAGS.has(typeFlag) && name.length > 0) {
      entries.push({ name, content: Buffer.from(tarBuffer.subarray(contentStart, contentStart + size)) });
    }
