...
const LANE_BY_LEGACY_PREFIX: ReadonlyMap<string, LaneName> = new Map(
  (Object.keys(CORE_LANES) as LaneName[])
    .map((lane) => {
      // `package ?? alias`: once a lane moves, `package` is null but guards and importers
      // still name it by its old package, and that name has to keep resolving until the
      // alias is removed.
