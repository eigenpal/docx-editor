...
import { caretContentX } from '../line-catalog.ts';
import { hasGeometryStopAtOffset } from '../navigation-stops.ts';
...
describe('engine layout IR -> contract display IR', () => {
  ...
  test('display bridge contains no production font family, size, or color defaults', () => {
    const source = readFileSync(new URL('../display-bridge.ts', import.meta.url), 'utf8');
