...
describe('character rStyle cascade and default character style', () => {
  ...
  test('character style toggles use XOR while direct formatting stays absolute', () => {
    const styles =
      ...
      `<w:rPr><w:b/><w:i/><w:caps/><w:smallCaps/><w:strike/><w:vanish/>` +
      `</w:rPr></w:style>` +
      `<w:style w:type="character" w:styleId="FalseAtTheTip">` +
    ...
    // The character level STATES false, so it is a level that stated the property, and
    // §17.7.3's XOR(paragraph true, character false) is true. The paragraph's bold survives.
    const falseAtTheTip = resolveRunStyle(
      cascadeRunProperties(
        inherited,
        [{ localName: 'rStyle', attributes: { val: 'FalseAtTheTip' } }],
        table
      )
    );
    expect(falseAtTheTip.bold).toBe(true);
  ...
  test('duplicate toggles inside ONE w:rPr state one value, the first', () => {
    // `CT_RPr`'s `EG_RPrBase` is a repeatable choice, so both of these are schema-valid.
    // §17.7.3 resolves a level by reading THE value in the style, so a second instance is not
    // a second level and cannot cancel the first.
    const styles =
    ...
    expect(boldFor('OffThenOn')).toBe(false);
  });

  test('a false at the tip of the character level IS that level, ending the basedOn walk', () => {
    // §17.7.3's walk reads the value in the style and only recurses into `basedOn` when the
    // style DOES NOT state it. `Cancel` states false, so `Base`'s true is never read and the
    // character level is false: XOR(paragraph true, character false) leaves the run bold.
    const styles =
      `<w:style w:type="paragraph" w:styleId="Heading"><w:rPr><w:b/></w:rPr></w:style>` +
    ...
    // And the base's true is reachable when the tip says nothing, which is the same walk.
    expect(
      resolveRunStyle(
        cascadeRunProperties(
          inherited,
          [{ localName: 'rStyle', attributes: { val: 'Base' } }],
          table
        )
      ).bold
    ).toBe(false);
