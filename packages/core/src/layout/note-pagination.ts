...
function bodyOnlyPage(page: PageRecord): PageRecord {
  // Identity-preserving when there is nothing to strip: most pages of a long document
  // carry no notes, and a fresh clone per page per pass destroyed the page identity every
  // downstream reuse check keys on.
  if (
...
export function attachNotesToLayout(
  ...
  let pages: PageRecord[] = layout.pages.map((page) => {
    ...
    // Identity through: an untouched page with no footnote area is the page itself, which
    // is what lets paint and the finalize restore skip it.
