...
/**
 * Vertical skip before an EMPTY line whose box intersects a side-channel wrap band.
 *
 * A text line shrinks around a square/tight/through float; an empty line has nothing to
 * shrink, so Word treats its full width as required and relocates the whole line below the
 * float instead of parking a paragraph-mark-only line in a channel. LibreOffice diverges
 * here and keeps empty lines beside the float — Word is the fidelity target. Same clearance
 * loop as {@link topAndBottomSkipBeforeLine}, over the OTHER wrap modes.
 */
export function sideWrapSkipBeforeEmptyLine(
  lineTopY: number,
  lineHeight: number,
  zones: readonly ExclusionZone[]
): number {
...
/** The pending-line fields the placement extent needs — structural, so break and placement agree. */
export interface PendingLineFlowShape {
  readonly spans: readonly unknown[];
  readonly drawings: readonly unknown[];
  readonly height: number;
  readonly trailingSpacing: number;
  readonly exclusionSkipBefore?: number;
}

/** Recompute exclusion skip at placement time from live page zones and absolute line top. */
export function pendingLineFlowExtentAtPlacement(
  lineTopY: number,
  line: PendingLineFlowShape,
  zones: readonly ExclusionZone[],
  tail = 0
): number {
  let skip = line.exclusionSkipBefore ?? 0;
  if (zones.length > 0) {
    skip = topAndBottomSkipBeforeLine(lineTopY, line.height, zones);
    // An empty line never shrinks into a wrap channel — see sideWrapSkipBeforeEmptyLine.
    if (line.spans.length === 0 && line.drawings.length === 0) {
      skip += sideWrapSkipBeforeEmptyLine(lineTopY + skip, line.height, zones);
    }
  }
