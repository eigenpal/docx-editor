    ...
    switch (operation.op) {
      ...
      case 'insertCustomNode': {
        ...
        if (hasAt === hasSpan) {
          return refuse(
            'unsupported-content',
            'a custom node goes at a position or over a span, not both',
            hasAt ? 'at+span' : 'no-place'
          );
        }
        if (typeof operation.tag !== 'string' || operation.tag.length === 0) {
          return refuse('unsupported-content', 'a custom node carries its identity in its tag');
        }
        ...
        if (operation.tag.length > MAX_CONTROL_TAG) {
          return refuse(
            'unsupported-content',
            `w:tag caps at ${String(MAX_CONTROL_TAG)} characters; a longer payload belongs in the store`,
            'tag'
          );
        }
        if (typeof operation.text !== 'string' || operation.text.length === 0) {
          return refuse('unsupported-content', 'a custom node shows some text', 'text');
        }
        if (operation.lock !== undefined && !CONTENT_CONTROL_LOCKS.has(operation.lock)) {
          return refuse('unsupported-content', 'that is not a lock', String(operation.lock));
        }
        ...
        if (operation.span) {
          const resolved = resolveSpanRef(operation.span, handles, packageReads);
          if (!resolved.ok)
            return refuse(resolved.code, 'that span is not a place', resolved.detail);
          if (!resolved.value)
            return refuse('invalid-offset', 'that story holds nothing to wrap', 'empty-story');
          range = resolved.value;
        } else {
          const resolved = resolvePoint(operation.at!, handles, packageReads);
          if (!resolved.ok) return refuse(resolved.code, 'that is not a place', resolved.detail);
          range = { start: resolved.value, end: resolved.value };
        }
        const story = storyReadsOf(range);
        if (!story) return refuse('invalid-handle', 'that story is not in this document');
        ...
        if (start.paragraphId !== end.paragraphId) {
          return refuse(
            'unsupported-content',
            'a custom node wraps text inside one paragraph',
            'multi-paragraph'
          );
        }
        ...
        return {
          ...
          write: {
            ...
            ...(operation.lock === undefined || operation.lock === 'unlocked'
              ? {}
              : { lock: operation.lock }),
            ...(payload.value === null ? {} : { payload: payload.value }),
          },
          answer: () => APPLIED,
        };
      }
