'use strict';
var chunkNGK4AG2P_js = require('./chunk-NGK4AG2P.js'),
  ...
  for (; s < r.length; ) {
    ...
    if (r.startsWith('<w:p', l)) {
      ...
      if (p === '>' || p === ' ' || p === '/') {
        ...
        if (g === -1) break;
        if (r[g - 1] === '/') {
          if (c === 0) return { start: i, end: g + 1 };
          s = g + 1;
        } else (c++, (s = g + 1));
      } else s = l + 1;
    } else if (r.startsWith('</w:p>', l)) {
      if ((c--, c === 0)) return { start: i, end: l + 6 };
      s = l + 6;
    } else s = l + 1;
  }
  return null;
...
function pt(r, t, e) {
  ...
  for (let s of e) {
    let c = n.get(s) || 0;
    if (c === 0) return { safe: false, reason: `paraId-not-found-in-original: ${s}` };
    if (c > 1) return { safe: false, reason: `duplicate-paraId-in-original: ${s}` };
  }
  for (let s of e) {
    let c = a.get(s) || 0;
    if (c === 0) return { safe: false, reason: `paraId-not-found-in-serialized: ${s}` };
    if (c > 1) return { safe: false, reason: `duplicate-paraId-in-serialized: ${s}` };
  }
  let o = Q(r),
    i = Q(t);
  return o !== i
    ? { safe: false, reason: `paragraph-count-mismatch: original=${o}, serialized=${i}` }
    : { safe: true };
}
function nt(r, t, e) {
  if (e.size === 0) return r;
  if (!pt(r, t, e).safe) return null;
  let a = [];
  for (let i of e) {
    let s = et(r, i);
    if (!s) return null;
    let c = ut(t, i);
    if (!c) return null;
    a.push({ start: s.start, end: s.end, newXml: c });
  }
  a.sort((i, s) => s.start - i.start);
  let o = r;
  for (let { start: i, end: s, newXml: c } of a) o = o.substring(0, i) + c + o.substring(s);
  return o;
}
function gt(r) {
  return r.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
...
function rt(r) {
  for (let t of r)
    if (t.type === 'paragraph') {
      ...
      for (let e of t.rows) for (let n of e.cells) if (rt(n.content)) return true;
    }
  return false;
}
async function at(r, t, e) {
  ...
  let i = r.package.document.content;
  if (rt(i)) return null;
  let s = r.package.document.comments,
    c = s && s.length > 0,
    l = chunk6L7HNODA_js.j(r);
  try {
    let g = await (await import('jszip')).default.loadAsync(t),
      f = new Map();
    if (n.size > 0) {
      let u = g.file('word/document.xml');
      if (!u) return null;
      let m = await u.async('text'),
        C = chunk6L7HNODA_js.c(r),
        d = nt(m, C, n);
      if (!d) return null;
      f.set('word/document.xml', d);
    }
    if (c) {
      f.set('word/comments.xml', chunk6L7HNODA_js.d(s));
      let u = g.file('[Content_Types].xml');
      if (u) {
        let d = await u.async('text');
        d.includes('/word/comments.xml') ||
          f.set(
            '[Content_Types].xml',
            d.replace(
              '</Types>',
              `<Override PartName="/word/comments.xml" ContentType="${chunk6L7HNODA_js.g}"/></Types>`
            )
          );
      }
      let m = 'word/_rels/document.xml.rels',
        C = g.file(m);
      if (C) {
        let d = await C.async('text');
        if (!d.includes('comments.xml')) {
          let y = chunk6L7HNODA_js.e(d);
          f.set(
            m,
            d.replace(
              '</Relationships>',
              `<Relationship Id="rId${y + 1}" Type="${chunk65V65RRX_js.a.comments}" Target="comments.xml"/></Relationships>`
            )
          );
        }
      }
    }
    for (let [u, m] of l) f.set(u, m);
    let x = g.file('docProps/core.xml');
    if (x) {
      let u = await x.async('text');
      f.set('docProps/core.xml', chunk6L7HNODA_js.k(u, { updateModifiedDate: !0 }));
    }
    return await chunk6L7HNODA_js.i(g, f);
  } catch {
    return null;
  }
}
...
function ae(r, t = {}) {
  return dt({ ...t, initialText: r });
}
var I = class r {
  constructor(t) {
    chunk5XGN7UAV_js.a(this, '_document');
    chunk5XGN7UAV_js.a(this, '_pendingVariables');
  ...
  static async fromBuffer(t) {
    let e = await chunk65V65RRX_js.u(t);
    return new r(e);
  }
  ...
  getVariables() {
    return chunkNGK4AG2P_js.a(this._document);
  }
  getStyles() {
  ...
  async applyVariables(t) {
    ...
    let { processTemplate: a } = await import('./processTemplate-P5AKLVQ5.js'),
      o = a(n, e),
      i = await chunk65V65RRX_js.u(o),
      s = new r(i);
  ...
  async toBuffer(t) {
    if (this._document.originalBuffer) {
      if (t?.selective) {
        let n = await at(this._document, this._document.originalBuffer, t.selective);
        if (n) return ((this._document.originalBuffer = n), n);
      }
      let e = await chunk6L7HNODA_js.f(this._document);
      return ((this._document.originalBuffer = e), e);
    }
    return chunk6L7HNODA_js.l(this._document);
  }
  ...
  executeCommands(t) {
    return new r(chunk4FIPZCBR_js.b(this._document, t));
  }
  _executeCommand(t) {
    let e = new r(chunk4FIPZCBR_js.a(this._document, t));
...
async function pe(r) {
  return I.fromBuffer(r);
}
function ge(r) {
  return I.fromDocument(r);
}
function ht(r, t = {}) {
...
function he(r, t, e = {}) {
  let { contextChars: n = 200, includeSuggestions: a = true } = e,
    i = r.package.document.content.filter((y) => y.type === 'paragraph'),
  ...
  if (t.start.paragraphIndex === t.end.paragraphIndex) l = c.slice(t.start.offset, t.end.offset);
    ...
    let y = [];
    for (let k = t.start.paragraphIndex; k <= t.end.paragraphIndex; k++) {
      let $ = i[k];
      if (!$) continue;
      let H = h($);
      k === t.start.paragraphIndex
        ? y.push(H.slice(t.start.offset))
        : k === t.end.paragraphIndex
          ? y.push(H.slice(0, t.end.offset))
          : y.push(H);
    }
    l = y.join(`
`);
  }
  let p = Dt(i, t.start.paragraphIndex, t.start.offset, n),
    g = wt(i, t.end.paragraphIndex, t.end.offset, n),
    f = vt(s, t.start.offset),
    x = s.formatting || {},
    u = {
    ...
    m = false,
    C = _t(s, t.start.offset),
    d = a ? Ht(l) : [];
  return {
    selectedText: l,
    range: t,
    formatting: f,
    paragraphFormatting: x,
    textBefore: p,
    textAfter: g,
    paragraph: u,
    inTable: m,
...
function ot(r) {
  let t = ht(r, { outlineMaxChars: 50, maxOutlineParagraphs: 10 }),
...
function h(r) {
  let t = [];
  for (let e of r.content) e.type === 'run' ? t.push(F(e)) : e.type === 'hyperlink' && t.push(E(e));
  return t.join('');
}
function F(r) {
  return r.content
...
function E(r) {
  let t = [];
  for (let e of r.children) e.type === 'run' && t.push(F(e));
  return t.join('');
}
function Pt(r) {
...
function Rt(r, t, e = {}) {
  let { contextCharsBefore: n = 200, contextCharsAfter: a = 200, includeSuggestions: o = true } = e,
    ...
    s = i.content.filter((y) => y.type === 'paragraph');
...
function be(r, t, e = {}) {
  let n = Rt(r, t, e),
    ...
    i = r.package.document.content.filter((u) => u.type === 'paragraph'),
    s = V(n.selectedText),
    c = n.selectedText.length,
    l = t.start.paragraphIndex !== t.end.paragraphIndex,
    p = [];
  for (let u = t.start.paragraphIndex; u <= t.end.paragraphIndex; u++) p.push(u);
  let g = jt(i, t),
    f = a ? ot(r) : void 0,
    x = Nt(n.selectedText);
  return {
...
function Ce(r, t) {
  ...
  for (let s = t.start.paragraphIndex; s <= t.end.paragraphIndex; s++) {
    let c = n[s];
    if (!c) continue;
    let l = s === t.start.paragraphIndex ? t.start.offset : 0,
      p = s === t.end.paragraphIndex ? t.end.offset : 1 / 0;
    Mt(c, l, p, a);
  }
  let o = a.length <= 1 || a.every((s) => Wt(s, a[0]));
...
function Bt(r, t) {
  if (t.start.paragraphIndex === t.end.paragraphIndex) {
    ...
    return n ? S(n).slice(t.start.offset, t.end.offset) : '';
  }
  let e = [];
...
function S(r) {
  let t = [];
  for (let e of r.content) e.type === 'run' ? t.push(P(e)) : e.type === 'hyperlink' && t.push(D(e));
  return t.join('');
}
function P(r) {
  return r.content
...
function D(r) {
  let t = [];
  for (let e of r.children) e.type === 'run' && t.push(P(e));
  return t.join('');
}
function V(r) {
...
function T(r) {
  let t = r.padStart(6, '0').slice(0, 6),
    e = parseInt(t.slice(0, 2), 16),
    n = parseInt(t.slice(2, 4), 16),
    a = parseInt(t.slice(4, 6), 16);
  return { r: isNaN(e) ? 0 : e, g: isNaN(n) ? 0 : n, b: isNaN(a) ? 0 : a };
}
function L(r, t, e) {
  let n = (a) =>
    Math.max(0, Math.min(255, Math.round(a)))
      .toString(16)
      .padStart(2, '0');
  return `${n(r)}${n(t)}${n(e)}`.toUpperCase();
}
function st(r, t, e) {
  ((r /= 255), (t /= 255), (e /= 255));
  let n = Math.max(r, t, e),
    a = Math.min(r, t, e),
    o = (n + a) / 2;
  if (n === a) return { h: 0, s: 0, l: o };
  let i = n - a,
    s = o > 0.5 ? i / (2 - n - a) : i / (n + a),
    c;
  switch (n) {
    case r:
      c = ((t - e) / i + (t < e ? 6 : 0)) / 6;
...
function ct(r, t, e) {
  ...
  let n = (i, s, c) => (
    ...
    o = 2 * e - a;
  return {
    r: Math.round(n(o, a, r + 1 / 3) * 255),
    g: Math.round(n(o, a, r) * 255),
    b: Math.round(n(o, a, r - 1 / 3) * 255),
  };
}
function v(r, t) {
  if (t <= 0 || t >= 1) return t >= 1 ? 'FFFFFF' : r;
  let e = T(r),
    n = st(e.r, e.g, e.b);
  n.l = n.l + (1 - n.l) * t;
  let a = ct(n.h, n.s, n.l);
  return L(a.r, a.g, a.b);
}
function _(r, t) {
  if (t <= 0 || t >= 1) return t <= 0 ? '000000' : r;
  let e = T(r),
    n = st(e.r, e.g, e.b);
  n.l = n.l * t;
  let a = ct(n.h, n.s, n.l);
...
function b(r, t, e = '000000') {
  if (!r) return `#${e}`;
  if (r.auto) return `#${e}`;
  let n;
  if (r.themeColor) {
    let a = lt(r.themeColor);
    if ((a ? (n = qt(t, a)) : (n = r.rgb ?? e), r.themeTint)) {
      let o = it(r.themeTint);
      n = v(n, o);
    } else if (r.themeShade) {
      let o = it(r.themeShade);
      n = _(n, o);
    }
  } else r.rgb ? (n = r.rgb === 'auto' ? e : r.rgb) : (n = e);
...
function Te(r, t) {
  return r ? (r.auto ? 'transparent' : b(r, t)) : '';
}
function ke(r, t) {
  if (!r) return false;
  if (r.auto) return true;
  let n = b(r, t).replace(/^#/, '').toLowerCase(),
    a = T(n);
  return (a.r + a.g + a.b) / 3 < 20;
}
function Pe(r, t) {
  if (!r) return false;
  let n = b(r, t).replace(/^#/, '').toLowerCase(),
    a = T(n);
  return (a.r + a.g + a.b) / 3 > 235;
}
function Ae(r, t) {
  if (!r) return '#000000';
  let n = b(r, t).replace(/^#/, ''),
    a = T(n);
  return (0.299 * a.r + 0.587 * a.g + 0.114 * a.b) / 255 > 0.5 ? '#000000' : '#FFFFFF';
}
function Ie(r) {
  if (!r) return;
  let t = r.trim();
...
function De(r) {
  return { rgb: r.replace(/^#/, '').toUpperCase() };
}
function we(r, t, e) {
  let a = b(r, t).replace(/^#/, ''),
    o = 1 - e / 100;
...
function Re(r, t, e) {
  return t === 'tint' ? v(r, e) : _(r, e);
}
function Be(r) {
  let t = r ?? w;
  return Yt.map((e) =>
    Kt.map((n) => {
      let a = t[n.slot] ?? w[n.slot] ?? '000000',
        o;
      e.type === 'base'
        ? (o = a.toUpperCase())
        : e.type === 'tint'
          ? (o = v(a, e.value))
          : (o = _(a, e.value));
      let i = { hex: o, themeSlot: n.slot, label: `${n.name}${e.labelSuffix}` };
      return (
...
exports.x = we;
exports.y = ve;
exports.z = _e; //# sourceMappingURL=chunk-XJCWBPY7.js.map
//# sourceMappingURL=chunk-XJCWBPY7.js.map
