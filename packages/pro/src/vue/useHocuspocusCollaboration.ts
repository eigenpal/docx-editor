...
/**
 * Test-only room factory. Not re-exported from `@docx-editor.dev/pro/vue/hocuspocus`.
 *
 * @internal
 */
...
function createRoomOf(
  options: UseHocuspocusCollaborationOptions | null | undefined
): HocuspocusCreateRoom {
  return (
    (options as InjectedHocuspocusCollaborationOptions | null | undefined)?.[
...
/**
 * Own a Hocuspocus collaboration room for a Vue host.
 *
 * The default Pro Vue entry does not import this composable. Import it from
 * `@docx-editor.dev/pro/vue/hocuspocus` so a review-only bundle does not load
 * a network provider.
 *
 * @public
 */
export function useHocuspocusCollaboration(
  options?: MaybeRefOrGetter<UseHocuspocusCollaborationOptions | null>
